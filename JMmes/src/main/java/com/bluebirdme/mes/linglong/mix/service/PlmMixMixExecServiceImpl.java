...
import com.bluebirdme.mes.linglong.mix.entity.PlmMixMixExec;
...
public class PlmMixMixExecServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmMixMixExec plmmixmixexec=(PlmMixMixExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmMixMixExec.class);
        		//plmmixmixexec.setCreated_time_t(new Date()); 
        		list.add(plmmixmixexec);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixMixExec plmmixmixexec =(PlmMixMixExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmMixMixExec.class);
        		//plmmixmixexec.setCreated_time_t(new Date()); 
        		list.add(plmmixmixexec);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixMixExec plmmixmixexec=(PlmMixMixExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmMixMixExec.class);
        		//plmmixmixexec.setCreated_time_t(new Date()); 
        		update(plmmixmixexec);
        	}
        }
