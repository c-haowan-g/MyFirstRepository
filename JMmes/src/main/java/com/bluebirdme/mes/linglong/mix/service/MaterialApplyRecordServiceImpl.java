...
import com.bluebirdme.mes.linglong.mix.entity.MaterialApplyRecord;
...
public class MaterialApplyRecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MaterialApplyRecord materialapplyrecord=(MaterialApplyRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MaterialApplyRecord.class);
        		//materialapplyrecord.setCreated_time_t(new Date()); 
        		list.add(materialapplyrecord);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MaterialApplyRecord materialapplyrecord =(MaterialApplyRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MaterialApplyRecord.class);
        		//materialapplyrecord.setCreated_time_t(new Date()); 
        		list.add(materialapplyrecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MaterialApplyRecord materialapplyrecord=(MaterialApplyRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MaterialApplyRecord.class);
        		//materialapplyrecord.setCreated_time_t(new Date()); 
        		update(materialapplyrecord);
        	}
        }
