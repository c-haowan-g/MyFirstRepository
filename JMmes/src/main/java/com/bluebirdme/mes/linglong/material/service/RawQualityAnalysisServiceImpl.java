...
import com.bluebirdme.mes.linglong.material.entity.RawQualityAnalysis;
...
public class RawQualityAnalysisServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return RawQualityAnalysis.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		RawQualityAnalysis rawqualityanalysis=(RawQualityAnalysis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), RawQualityAnalysis.class);
        		//rawqualityanalysis.setCreated_time_t(new Date()); 
        		list.add(rawqualityanalysis);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		RawQualityAnalysis rawqualityanalysis =(RawQualityAnalysis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), RawQualityAnalysis.class);
        		//rawqualityanalysis.setCreated_time_t(new Date()); 
        		list.add(rawqualityanalysis);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		RawQualityAnalysis rawqualityanalysis=(RawQualityAnalysis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), RawQualityAnalysis.class);
        		//rawqualityanalysis.setCreated_time_t(new Date()); 
        		update(rawqualityanalysis);
        	}
        }
