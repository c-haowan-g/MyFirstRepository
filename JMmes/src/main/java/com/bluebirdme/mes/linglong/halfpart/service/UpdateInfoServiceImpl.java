...
import com.bluebirdme.mes.linglong.halfpart.entity.UpdateInfo;
...
public class UpdateInfoServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		UpdateInfo updateinfo=(UpdateInfo) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), UpdateInfo.class);
        		//updateinfo.setCreated_time_t(new Date()); 
        		list.add(updateinfo);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		UpdateInfo updateinfo =(UpdateInfo) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), UpdateInfo.class);
        		//updateinfo.setCreated_time_t(new Date()); 
        		list.add(updateinfo);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		UpdateInfo updateinfo=(UpdateInfo) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), UpdateInfo.class);
        		//updateinfo.setCreated_time_t(new Date()); 
        		update(updateinfo);
        	}
        }
