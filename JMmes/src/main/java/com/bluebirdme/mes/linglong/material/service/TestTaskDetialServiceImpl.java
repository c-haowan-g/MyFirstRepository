...
public class TestTaskDetialServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		TestTaskDetial testtaskdetial=(TestTaskDetial) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), TestTaskDetial.class);
        		//testtaskdetial.setCreated_time_t(new Date()); 
        		list.add(testtaskdetial);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		TestTaskDetial testtaskdetial =(TestTaskDetial) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), TestTaskDetial.class);
        		//testtaskdetial.setCreated_time_t(new Date()); 
        		list.add(testtaskdetial);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		TestTaskDetial testtaskdetial=(TestTaskDetial) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), TestTaskDetial.class);
        		//testtaskdetial.setCreated_time_t(new Date()); 
        		update(testtaskdetial);
        	}
        }
