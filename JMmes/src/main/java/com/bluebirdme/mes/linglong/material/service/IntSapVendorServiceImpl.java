...
import com.bluebirdme.mes.linglong.material.entity.IntSapVendor;
...
public class IntSapVendorServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		IntSapVendor intsapvendor=(IntSapVendor) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), IntSapVendor.class);
        		//intsapvendor.setCreated_time_t(new Date()); 
        		list.add(intsapvendor);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		IntSapVendor intsapvendor =(IntSapVendor) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), IntSapVendor.class);
        		//intsapvendor.setCreated_time_t(new Date()); 
        		list.add(intsapvendor);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		IntSapVendor intsapvendor=(IntSapVendor) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), IntSapVendor.class);
        		//intsapvendor.setCreated_time_t(new Date()); 
        		update(intsapvendor);
        	}
        }
