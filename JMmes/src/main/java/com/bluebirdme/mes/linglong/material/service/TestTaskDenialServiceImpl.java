...
public class TestTaskDenialServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		TestTaskDenial testtaskdenial=(TestTaskDenial) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), TestTaskDenial.class);
        		//testtaskdenial.setCreated_time_t(new Date()); 
        		list.add(testtaskdenial);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		TestTaskDenial testtaskdenial =(TestTaskDenial) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), TestTaskDenial.class);
        		//testtaskdenial.setCreated_time_t(new Date()); 
        		list.add(testtaskdenial);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		TestTaskDenial testtaskdenial=(TestTaskDenial) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), TestTaskDenial.class);
        		//testtaskdenial.setCreated_time_t(new Date()); 
        		update(testtaskdenial);
        	}
        }
