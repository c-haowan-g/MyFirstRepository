...
public class MixReportByDismantlingServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MixReportByDismantling.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixReportByDismantling mixreportbydismantling=(MixReportByDismantling) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixReportByDismantling.class);
        		//mixreportbydismantling.setCreated_time_t(new Date()); 
        		list.add(mixreportbydismantling);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixReportByDismantling mixreportbydismantling =(MixReportByDismantling) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixReportByDismantling.class);
        		//mixreportbydismantling.setCreated_time_t(new Date()); 
        		list.add(mixreportbydismantling);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixReportByDismantling mixreportbydismantling=(MixReportByDismantling) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixReportByDismantling.class);
        		//mixreportbydismantling.setCreated_time_t(new Date()); 
        		update(mixreportbydismantling);
        	}
        }
