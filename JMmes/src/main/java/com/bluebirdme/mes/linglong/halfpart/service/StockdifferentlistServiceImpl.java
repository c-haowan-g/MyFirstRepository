...
import com.bluebirdme.mes.linglong.halfpart.entity.Stockdifferentlist;
...
public class StockdifferentlistServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return Stockdifferentlist.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Stockdifferentlist stockdifferentlist=(Stockdifferentlist) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Stockdifferentlist.class);
        		//stockdifferentlist.setCreated_time_t(new Date()); 
        		list.add(stockdifferentlist);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Stockdifferentlist stockdifferentlist =(Stockdifferentlist) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Stockdifferentlist.class);
        		//stockdifferentlist.setCreated_time_t(new Date()); 
        		list.add(stockdifferentlist);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Stockdifferentlist stockdifferentlist=(Stockdifferentlist) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Stockdifferentlist.class);
        		//stockdifferentlist.setCreated_time_t(new Date()); 
        		update(stockdifferentlist);
        	}
        }
