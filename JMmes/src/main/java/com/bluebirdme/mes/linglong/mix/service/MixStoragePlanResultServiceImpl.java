...
public class MixStoragePlanResultServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MixStoragePlanResult.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixStoragePlanResult mixstorageplanresult=(MixStoragePlanResult) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixStoragePlanResult.class);
        		//mixstorageplanresult.setCreated_time_t(new Date()); 
        		list.add(mixstorageplanresult);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixStoragePlanResult mixstorageplanresult =(MixStoragePlanResult) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixStoragePlanResult.class);
        		//mixstorageplanresult.setCreated_time_t(new Date()); 
        		list.add(mixstorageplanresult);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixStoragePlanResult mixstorageplanresult=(MixStoragePlanResult) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixStoragePlanResult.class);
        		//mixstorageplanresult.setCreated_time_t(new Date()); 
        		update(mixstorageplanresult);
        	}
        }
