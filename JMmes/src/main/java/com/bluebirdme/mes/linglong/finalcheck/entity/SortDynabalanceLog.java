 ...
 */
@ATDefinition("D_QM_SORTDYNABALANCELOG")
@Entity
@XmlRootElement
@Table(name="AT_D_QM_SORTDYNABALANCELOG")
public class SortDynabalanceLog extends RockWellBaseEntity{
    ...
    @Comment("层：1-1#(层)、2-2#(层)、3-3#(层)、4-4#(层)")
    @Column(nullable=true,length=0)
    private Integer floor_i;
    
    @Comment("质量等级编码")
    @Column(nullable=true,length=0)
    private Integer gradecd_i;
    
    @Comment("分拣是否成功:0-失败，1-成功")
    @Column(nullable=true,length=0)
    ...
    public Integer getFloor_i(){
        return floor_i;
    }
    
    @XmlElement
    public void setFloor_i(Integer floor_i){
        this.floor_i=floor_i;
    }
