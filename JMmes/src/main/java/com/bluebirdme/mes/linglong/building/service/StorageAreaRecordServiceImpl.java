...
import com.bluebirdme.mes.linglong.building.entity.StorageAreaRecord;
 ...
 * 成型线边库操作履历
 * @author 李昂
 * @Date 2020-11-25 08:05:38
 */
@Service
@Transactional(rollbackFor=Exception.class)
public class StorageAreaRecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return StorageAreaRecord.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		StorageAreaRecord storagearearecord=(StorageAreaRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), StorageAreaRecord.class);
        		//storagearearecord.setCreated_time_t(new Date()); 
        		list.add(storagearearecord);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		StorageAreaRecord storagearearecord =(StorageAreaRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), StorageAreaRecord.class);
        		//storagearearecord.setCreated_time_t(new Date()); 
        		list.add(storagearearecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		StorageAreaRecord storagearearecord=(StorageAreaRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), StorageAreaRecord.class);
        		//storagearearecord.setCreated_time_t(new Date()); 
        		update(storagearearecord);
        	}
        }
