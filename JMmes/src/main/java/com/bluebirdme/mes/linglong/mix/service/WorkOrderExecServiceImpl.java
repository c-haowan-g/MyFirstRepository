...
import com.bluebirdme.mes.linglong.mix.entity.WorkOrderExec;
...
public class WorkOrderExecServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public Map<String, Object> datagridUDAjl(Filter filter, Page page) throws Exception {
	...
	public List<Map<String, Object>> datagridRowsUDAjl(Filter filter, Page page) throws Exception {
		if (getSQL("datagridUDA_rowsjl") == null)
			throw new Exception(getClass() + ".xml中未找到ID为datagridUDA_rowsjl的SQL语句定义");
	...
	public Long datagridTotalUDAjl(Filter filter, Page page) throws Exception {
		...
		if (getSQL("datagridUDA_totaljl") == null)
			throw new Exception(getClass() + ".xml中未找到ID为datagridUDA_totaljl的SQL语句定义");
	...
	public Map<String, Object> datagridUDAxl(Filter filter, Page page) throws Exception {
	...
	@SuppressWarnings({ "unchecked" })
	public List<Map<String, Object>> datagridRowsUDAxl(Filter filter, Page page) throws Exception {
		if (getSQL("datagridUDA_rowsxl") == null)
			throw new Exception(getClass() + ".xml中未找到ID为datagridUDA_rowsxl的SQL语句定义");
	...
	public Long datagridTotalUDAxl(Filter filter, Page page) throws Exception {
		...
		if (getSQL("datagridUDA_totalxl") == null)
			throw new Exception(getClass() + ".xml中未找到ID为datagridUDA_totalxl的SQL语句定义");
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		WorkOrderExec workorderexec=(WorkOrderExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), WorkOrderExec.class);
        		//workorderexec.setCreated_time_t(new Date()); 
        		list.add(workorderexec);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		WorkOrderExec workorderexec =(WorkOrderExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), WorkOrderExec.class);
        		//workorderexec.setCreated_time_t(new Date()); 
        		list.add(workorderexec);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		WorkOrderExec workorderexec=(WorkOrderExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), WorkOrderExec.class);
        		//workorderexec.setCreated_time_t(new Date()); 
        		update(workorderexec);
        	}
        }
