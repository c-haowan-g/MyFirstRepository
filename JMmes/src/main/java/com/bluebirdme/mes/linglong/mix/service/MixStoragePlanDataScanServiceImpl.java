...
import com.bluebirdme.mes.linglong.mix.entity.MixStoragePlanDataScan;
...
public class MixStoragePlanDataScanServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MixStoragePlanDataScan.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixStoragePlanDataScan mixstorageplandatascan=(MixStoragePlanDataScan) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixStoragePlanDataScan.class);
        		//mixstorageplandatascan.setCreated_time_t(new Date()); 
        		list.add(mixstorageplandatascan);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixStoragePlanDataScan mixstorageplandatascan =(MixStoragePlanDataScan) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixStoragePlanDataScan.class);
        		//mixstorageplandatascan.setCreated_time_t(new Date()); 
        		list.add(mixstorageplandatascan);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixStoragePlanDataScan mixstorageplandatascan=(MixStoragePlanDataScan) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixStoragePlanDataScan.class);
        		//mixstorageplandatascan.setCreated_time_t(new Date()); 
        		update(mixstorageplandatascan);
        	}
        }
