...
import com.bluebirdme.mes.linglong.halfpart.entity.CalibRationRecord;
...
public class CalibRationRecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		CalibRationRecord calibrationrecord=(CalibRationRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), CalibRationRecord.class);
        		//calibrationrecord.setCreated_time_t(new Date()); 
        		list.add(calibrationrecord);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		CalibRationRecord calibrationrecord =(CalibRationRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), CalibRationRecord.class);
        		//calibrationrecord.setCreated_time_t(new Date()); 
        		list.add(calibrationrecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		CalibRationRecord calibrationrecord=(CalibRationRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), CalibRationRecord.class);
        		//calibrationrecord.setCreated_time_t(new Date()); 
