...
import com.bluebirdme.mes.linglong.mix.entity.PlmMixMixEdit;
...
public class PlmMixMixEditServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmMixMixEdit plmmixmixedit=(PlmMixMixEdit) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmMixMixEdit.class);
        		//plmmixmixedit.setCreated_time_t(new Date()); 
        		list.add(plmmixmixedit);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixMixEdit plmmixmixedit =(PlmMixMixEdit) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmMixMixEdit.class);
        		//plmmixmixedit.setCreated_time_t(new Date()); 
        		list.add(plmmixmixedit);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixMixEdit plmmixmixedit=(PlmMixMixEdit) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmMixMixEdit.class);
        		//plmmixmixedit.setCreated_time_t(new Date()); 
        		update(plmmixmixedit);
        	}
        }
