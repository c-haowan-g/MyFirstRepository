...
public class MixProductReportServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixProductReport mixproductreport=(MixProductReport) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixProductReport.class);
        		//mixproductreport.setCreated_time_t(new Date()); 
        		list.add(mixproductreport);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixProductReport mixproductreport =(MixProductReport) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixProductReport.class);
        		//mixproductreport.setCreated_time_t(new Date()); 
        		list.add(mixproductreport);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixProductReport mixproductreport=(MixProductReport) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixProductReport.class);
        		//mixproductreport.setCreated_time_t(new Date()); 
        		update(mixproductreport);
        	}
        }
	...
	public Long datagridTotalUDA3(Filter filter, Page page) throws Exception {
	   ...
	   public Map<String, Object> datagridUDAhis(Filter filter, Page page) throws Exception {
			...
			Map<String, Object> ret = new HashMap<String, Object>();
			ret.put("rows", datagridRowsUDAhis(filter, page));
			filter.setOrder(null);
			filter.setSort(null);
			ret.put("total", datagridTotalUDAhis(filter, page));
			return ret;
		}
	    
	    @SuppressWarnings({ "unchecked" })
		public List<Map<String, Object>> datagridRowsUDAhis(Filter filter, Page page) throws Exception {
			if (getSQL("datagridUDA_rowshis") == null)
				throw new Exception(getClass() + ".xml中未找到ID为datagridUDA_rowshis的SQL语句定义");

			String dataSql = getSQL("datagridUDA_rowshis", filter.getFilter());
			System.out.println(dataSql);
		...
		public Long datagridTotalUDAhis(Filter filter, Page page) throws Exception {
			if (getSQL("datagridUDA_totalhis") == null)
				throw new Exception(getClass() + ".xml中未找到ID为datagridUDA_totalhis的SQL语句定义");
			String totalSql = getSQL("datagridUDA_totalhis", filter.getFilter());
    ...
    public Map<String, Object> reportFirst(Filter filter, Page page)
    	    throws Exception
    	  {
    	    return getAll1(filter, page);
    	  }

    	  public Map<String, Object> getAll1(Filter filter, Page page)
    	    throws Exception
    	  {
    	    return datagrid2(filter, page, null);
    	  }
