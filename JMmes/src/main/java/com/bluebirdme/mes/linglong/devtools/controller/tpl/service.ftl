...
public class ${entityName}ServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		${entityName} ${entityName?lower_case}=(${entityName}) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), ${entityName}.class);
        		//${entityName?lower_case}.setCreated_time_t(new Date()); 
        		list.add(${entityName?lower_case});
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		${entityName} ${entityName?lower_case} =(${entityName}) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), ${entityName}.class);
        		//${entityName?lower_case}.setCreated_time_t(new Date()); 
        		list.add(${entityName?lower_case});
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		${entityName} ${entityName?lower_case}=(${entityName}) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), ${entityName}.class);
        		//${entityName?lower_case}.setCreated_time_t(new Date()); 
        		update(${entityName?lower_case});
        	}
        }
