...
public class MaterialsExecServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MaterialsExec materialsexec=(MaterialsExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MaterialsExec.class);
        		//materialsexec.setCreated_time_t(new Date()); 
        		list.add(materialsexec);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MaterialsExec materialsexec =(MaterialsExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MaterialsExec.class);
        		//materialsexec.setCreated_time_t(new Date()); 
        		list.add(materialsexec);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MaterialsExec materialsexec=(MaterialsExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MaterialsExec.class);
        		//materialsexec.setCreated_time_t(new Date()); 
        		update(materialsexec);
        	}
        }
