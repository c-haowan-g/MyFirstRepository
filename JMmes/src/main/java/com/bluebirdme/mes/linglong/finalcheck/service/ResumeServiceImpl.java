...
import com.bluebirdme.mes.linglong.finalcheck.entity.Resume;
...
public class ResumeServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Resume resume=(Resume) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Resume.class);
        		//resume.setCreated_time_t(new Date()); 
        		list.add(resume);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Resume resume =(Resume) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Resume.class);
        		//resume.setCreated_time_t(new Date()); 
        		list.add(resume);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Resume resume=(Resume) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Resume.class);
        		//resume.setCreated_time_t(new Date()); 
        		update(resume);
        	}
        }
