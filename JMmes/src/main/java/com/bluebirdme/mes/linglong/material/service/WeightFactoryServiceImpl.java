...
import com.bluebirdme.mes.linglong.material.entity.WeightFactory;
...
public class WeightFactoryServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		WeightFactory weightfactory=(WeightFactory) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), WeightFactory.class);
        		//weightfactory.setCreated_time_t(new Date()); 
        		list.add(weightfactory);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		WeightFactory weightfactory =(WeightFactory) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), WeightFactory.class);
        		//weightfactory.setCreated_time_t(new Date()); 
        		list.add(weightfactory);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		WeightFactory weightfactory=(WeightFactory) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), WeightFactory.class);
        		//weightfactory.setCreated_time_t(new Date()); 
        		update(weightfactory);
        	}
        }
