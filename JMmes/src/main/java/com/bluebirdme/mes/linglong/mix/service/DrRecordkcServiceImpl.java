...
import com.bluebirdme.mes.linglong.mix.entity.DrRecordkc;
...
public class DrRecordkcServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		DrRecordkc drrecordkc=(DrRecordkc) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), DrRecordkc.class);
        		//drrecordkc.setCreated_time_t(new Date()); 
        		list.add(drrecordkc);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		DrRecordkc drrecordkc =(DrRecordkc) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), DrRecordkc.class);
        		//drrecordkc.setCreated_time_t(new Date()); 
        		list.add(drrecordkc);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		DrRecordkc drrecordkc=(DrRecordkc) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), DrRecordkc.class);
        		//drrecordkc.setCreated_time_t(new Date()); 
        		update(drrecordkc);
        	}
        }
