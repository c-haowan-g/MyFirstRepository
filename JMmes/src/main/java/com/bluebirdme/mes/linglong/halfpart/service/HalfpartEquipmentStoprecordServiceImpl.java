...
import com.bluebirdme.mes.linglong.halfpart.entity.HalfpartEquipmentStoprecord;
...
public class HalfpartEquipmentStoprecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return HalfpartEquipmentStoprecord.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		HalfpartEquipmentStoprecord halfpartequipmentstoprecord=(HalfpartEquipmentStoprecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), HalfpartEquipmentStoprecord.class);
        		//halfpartequipmentstoprecord.setCreated_time_t(new Date()); 
        		list.add(halfpartequipmentstoprecord);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		HalfpartEquipmentStoprecord halfpartequipmentstoprecord =(HalfpartEquipmentStoprecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), HalfpartEquipmentStoprecord.class);
        		//halfpartequipmentstoprecord.setCreated_time_t(new Date()); 
        		list.add(halfpartequipmentstoprecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		HalfpartEquipmentStoprecord halfpartequipmentstoprecord=(HalfpartEquipmentStoprecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), HalfpartEquipmentStoprecord.class);
        		//halfpartequipmentstoprecord.setCreated_time_t(new Date()); 
        		update(halfpartequipmentstoprecord);
        	}
        }
