...
public class ReturnPolyRecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		ReturnPolyRecord returnpolyrecord=(ReturnPolyRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), ReturnPolyRecord.class);
        		//returnpolyrecord.setCreated_time_t(new Date()); 
        		list.add(returnpolyrecord);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		ReturnPolyRecord returnpolyrecord =(ReturnPolyRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), ReturnPolyRecord.class);
        		//returnpolyrecord.setCreated_time_t(new Date()); 
        		list.add(returnpolyrecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		ReturnPolyRecord returnpolyrecord=(ReturnPolyRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), ReturnPolyRecord.class);
        		//returnpolyrecord.setCreated_time_t(new Date()); 
        		update(returnpolyrecord);
        	}
        }
