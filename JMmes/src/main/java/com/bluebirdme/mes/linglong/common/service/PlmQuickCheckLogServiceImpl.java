...
import com.bluebirdme.mes.linglong.common.entity.PlmQuickCheckLog;
...
public class PlmQuickCheckLogServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmQuickCheckLog plmquickchecklog=(PlmQuickCheckLog) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmQuickCheckLog.class);
        		//plmquickchecklog.setCreated_time_t(new Date()); 
        		list.add(plmquickchecklog);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmQuickCheckLog plmquickchecklog =(PlmQuickCheckLog) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmQuickCheckLog.class);
        		//plmquickchecklog.setCreated_time_t(new Date()); 
        		list.add(plmquickchecklog);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmQuickCheckLog plmquickchecklog=(PlmQuickCheckLog) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmQuickCheckLog.class);
        		//plmquickchecklog.setCreated_time_t(new Date()); 
        		update(plmquickchecklog);
        	}
        }
