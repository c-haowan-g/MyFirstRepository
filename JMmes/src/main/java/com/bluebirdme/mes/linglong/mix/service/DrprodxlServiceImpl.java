...
import com.bluebirdme.mes.linglong.mix.entity.Drprodxl;
...
public class DrprodxlServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Drprodxl drprodxl=(Drprodxl) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Drprodxl.class);
        		//drprodxl.setCreated_time_t(new Date()); 
        		list.add(drprodxl);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Drprodxl drprodxl =(Drprodxl) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Drprodxl.class);
        		//drprodxl.setCreated_time_t(new Date()); 
        		list.add(drprodxl);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Drprodxl drprodxl=(Drprodxl) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Drprodxl.class);
        		//drprodxl.setCreated_time_t(new Date()); 
        		update(drprodxl);
        	}
        }
