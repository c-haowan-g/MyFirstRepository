...
import com.bluebirdme.mes.linglong.material.entity.MaterialStoragePlanDataScan;
...
public class MaterialStoragePlanDataScanServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MaterialStoragePlanDataScan.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MaterialStoragePlanDataScan materialstorageplandatascan=(MaterialStoragePlanDataScan) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MaterialStoragePlanDataScan.class);
        		//materialstorageplandatascan.setCreated_time_t(new Date()); 
        		list.add(materialstorageplandatascan);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStoragePlanDataScan materialstorageplandatascan =(MaterialStoragePlanDataScan) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MaterialStoragePlanDataScan.class);
        		//materialstorageplandatascan.setCreated_time_t(new Date()); 
        		list.add(materialstorageplandatascan);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStoragePlanDataScan materialstorageplandatascan=(MaterialStoragePlanDataScan) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MaterialStoragePlanDataScan.class);
        		//materialstorageplandatascan.setCreated_time_t(new Date()); 
        		update(materialstorageplandatascan);
        	}
        }
