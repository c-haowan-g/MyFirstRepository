...
import com.bluebirdme.mes.linglong.mix.entity.PlmMixWeightEdit;
...
public class PlmMixWeightEditServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmMixWeightEdit plmmixweightedit=(PlmMixWeightEdit) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmMixWeightEdit.class);
        		//plmmixweightedit.setCreated_time_t(new Date()); 
        		list.add(plmmixweightedit);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixWeightEdit plmmixweightedit =(PlmMixWeightEdit) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmMixWeightEdit.class);
        		//plmmixweightedit.setCreated_time_t(new Date()); 
        		list.add(plmmixweightedit);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixWeightEdit plmmixweightedit=(PlmMixWeightEdit) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmMixWeightEdit.class);
        		//plmmixweightedit.setCreated_time_t(new Date()); 
        		update(plmmixweightedit);
        	}
        }
