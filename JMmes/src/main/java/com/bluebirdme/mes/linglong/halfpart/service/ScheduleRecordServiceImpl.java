...
import com.bluebirdme.mes.linglong.halfpart.entity.ScheduleRecord;
...
public class ScheduleRecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		ScheduleRecord schedulerecord=(ScheduleRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), ScheduleRecord.class);
        		//schedulerecord.setCreated_time_t(new Date()); 
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		ScheduleRecord schedulerecord =(ScheduleRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), ScheduleRecord.class);
        		//schedulerecord.setCreated_time_t(new Date()); 
        		list.add(schedulerecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		ScheduleRecord schedulerecord=(ScheduleRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), ScheduleRecord.class);
        		//schedulerecord.setCreated_time_t(new Date()); 
    ...
    public List<Map<String, Object>> searchLine() throws Exception{
    	SQLQuery query = createSQLQuery(getSQL("searchLine"));
		return query.list();
    }
    public List<Map<String, Object>> searchLine2() throws Exception{
    	SQLQuery query = createSQLQuery(getSQL("searchLine2"));
		return query.list();
    }
}
