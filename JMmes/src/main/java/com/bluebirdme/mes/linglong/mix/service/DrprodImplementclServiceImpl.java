...
import com.bluebirdme.mes.linglong.mix.entity.DrprodImplementcl;
...
public class DrprodImplementclServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return DrprodImplementcl.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		DrprodImplementcl drprodimplementcl=(DrprodImplementcl) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), DrprodImplementcl.class);
        		//drprodimplementcl.setCreated_time_t(new Date()); 
        		list.add(drprodimplementcl);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		DrprodImplementcl drprodimplementcl =(DrprodImplementcl) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), DrprodImplementcl.class);
        		//drprodimplementcl.setCreated_time_t(new Date()); 
        		list.add(drprodimplementcl);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		DrprodImplementcl drprodimplementcl=(DrprodImplementcl) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), DrprodImplementcl.class);
        		//drprodimplementcl.setCreated_time_t(new Date()); 
        		update(drprodimplementcl);
        	}
        }
