...
public class MixSiloMaterialExecServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MixSiloMaterialExec.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixSiloMaterialExec mixsilomaterialexec=(MixSiloMaterialExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixSiloMaterialExec.class);
        		//mixsilomaterialexec.setCreated_time_t(new Date()); 
        		list.add(mixsilomaterialexec);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixSiloMaterialExec mixsilomaterialexec =(MixSiloMaterialExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixSiloMaterialExec.class);
        		//mixsilomaterialexec.setCreated_time_t(new Date()); 
        		list.add(mixsilomaterialexec);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixSiloMaterialExec mixsilomaterialexec=(MixSiloMaterialExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixSiloMaterialExec.class);
        		//mixsilomaterialexec.setCreated_time_t(new Date()); 
        		update(mixsilomaterialexec);
        	}
        }
