...
import com.bluebirdme.mes.linglong.mix.entity.Drrequest;
...
public class DrrequestServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Drrequest drrequest=(Drrequest) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Drrequest.class);
        		//drrequest.setCreated_time_t(new Date()); 
        		list.add(drrequest);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Drrequest drrequest =(Drrequest) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Drrequest.class);
        		//drrequest.setCreated_time_t(new Date()); 
        		list.add(drrequest);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Drrequest drrequest=(Drrequest) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Drrequest.class);
        		//drrequest.setCreated_time_t(new Date()); 
        		update(drrequest);
        	}
        }
