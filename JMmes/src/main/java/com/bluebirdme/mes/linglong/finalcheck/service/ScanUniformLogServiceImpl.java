...
import com.bluebirdme.mes.linglong.finalcheck.entity.ScanUniformLog;
...
public class ScanUniformLogServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		ScanUniformLog scanuniformlog=(ScanUniformLog) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), ScanUniformLog.class);
        		//scanuniformlog.setCreated_time_t(new Date()); 
        		list.add(scanuniformlog);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		ScanUniformLog scanuniformlog =(ScanUniformLog) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), ScanUniformLog.class);
        		//scanuniformlog.setCreated_time_t(new Date()); 
        		list.add(scanuniformlog);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		ScanUniformLog scanuniformlog=(ScanUniformLog) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), ScanUniformLog.class);
        		//scanuniformlog.setCreated_time_t(new Date()); 
        		update(scanuniformlog);
        	}
        }
