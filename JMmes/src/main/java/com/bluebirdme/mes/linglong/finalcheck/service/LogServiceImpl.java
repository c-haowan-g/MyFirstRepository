...
import com.bluebirdme.mes.linglong.finalcheck.entity.Log;
...
public class LogServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Log log=(Log) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Log.class);
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Log log =(Log) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Log.class);
        		//log.setCreated_time_t(new Date()); 
        		list.add(log);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Log log=(Log) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Log.class);
        		//log.setCreated_time_t(new Date()); 
        		update(log);
        	}
        }
