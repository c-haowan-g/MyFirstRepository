...
import com.bluebirdme.mes.linglong.building.entity.TireUpdateResume;
...
public class TireUpdateResumeServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		TireUpdateResume tireupdateresume=(TireUpdateResume) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), TireUpdateResume.class);
        		//tireupdateresume.setCreated_time_t(new Date()); 
        		list.add(tireupdateresume);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		TireUpdateResume tireupdateresume =(TireUpdateResume) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), TireUpdateResume.class);
        		//tireupdateresume.setCreated_time_t(new Date()); 
        		list.add(tireupdateresume);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		TireUpdateResume tireupdateresume=(TireUpdateResume) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), TireUpdateResume.class);
        		//tireupdateresume.setCreated_time_t(new Date()); 
        		update(tireupdateresume);
        	}
        }
