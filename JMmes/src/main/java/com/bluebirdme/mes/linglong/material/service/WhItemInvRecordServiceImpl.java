...
import com.bluebirdme.mes.linglong.material.entity.WhItemInvRecord;
...
public class WhItemInvRecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		WhItemInvRecord whiteminvrecord=(WhItemInvRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), WhItemInvRecord.class);
        		//whiteminvrecord.setCreated_time_t(new Date()); 
        		list.add(whiteminvrecord);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		WhItemInvRecord whiteminvrecord =(WhItemInvRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), WhItemInvRecord.class);
        		//whiteminvrecord.setCreated_time_t(new Date()); 
        		list.add(whiteminvrecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		WhItemInvRecord whiteminvrecord=(WhItemInvRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), WhItemInvRecord.class);
        		//whiteminvrecord.setCreated_time_t(new Date()); 
        		update(whiteminvrecord);
        	}
        }
