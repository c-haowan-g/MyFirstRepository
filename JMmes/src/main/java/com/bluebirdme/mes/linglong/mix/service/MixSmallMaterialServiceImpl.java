...
public class MixSmallMaterialServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixSmallMaterial mixsmallmaterial=(MixSmallMaterial) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixSmallMaterial.class);
        		//mixsmallmaterial.setCreated_time_t(new Date()); 
        		list.add(mixsmallmaterial);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixSmallMaterial mixsmallmaterial =(MixSmallMaterial) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixSmallMaterial.class);
        		//mixsmallmaterial.setCreated_time_t(new Date()); 
        		list.add(mixsmallmaterial);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixSmallMaterial mixsmallmaterial=(MixSmallMaterial) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixSmallMaterial.class);
        		//mixsmallmaterial.setCreated_time_t(new Date()); 
        		update(mixsmallmaterial);
        	}
        }
