...
import com.bluebirdme.mes.linglong.mix.entity.PlmMixMixExecHis;
...
public class PlmMixMixExecHisServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmMixMixExecHis plmmixmixexechis=(PlmMixMixExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmMixMixExecHis.class);
        		//plmmixmixexechis.setCreated_time_t(new Date()); 
        		list.add(plmmixmixexechis);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixMixExecHis plmmixmixexechis =(PlmMixMixExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmMixMixExecHis.class);
        		//plmmixmixexechis.setCreated_time_t(new Date()); 
        		list.add(plmmixmixexechis);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixMixExecHis plmmixmixexechis=(PlmMixMixExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmMixMixExecHis.class);
        		//plmmixmixexechis.setCreated_time_t(new Date()); 
        		update(plmmixmixexechis);
        	}
        }
