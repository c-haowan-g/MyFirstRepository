...
public class CarWashServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		CarWash carwash=(CarWash) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), CarWash.class);
        		//carwash.setCreated_time_t(new Date()); 
        		list.add(carwash);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        		...
        		carwash.setS_factory_s("0");
        		carwash.setCreated_by_s(WebUtils.getUser().userCode);
        		carwash.setCreated_name_s(WebUtils.getUser().userName);
        		carwash.setCreated_time_t(new Date()); 
        		list.add(carwash);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		CarWash carwash=(CarWash) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), CarWash.class);
        		carwash.setChanged_by_s(WebUtils.getUser().userCode);
        		carwash.setChanged_name_s(WebUtils.getUser().userName);
        		carwash.setChanged_time_t(new Date());
        		update(carwash);
        	}
        }
    
    }
    public List<Map<String, Object>> getRecipeInfo() {
        SQLQuery query = getSession().createSQLQuery(getSQL("getRecipeInfo"));
        query.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP);
        return query.list();
      }

      public List<Map<String, Object>> getWashRecipeInfo(String mater_type_s, String recipe_code_s) {
        SQLQuery query = getSession().createSQLQuery(getSQL("getWashRecipeInfo"));
        query.setParameter("mater_type_s", mater_type_s);
        query.setParameter("recipe_code_s", recipe_code_s);
        query.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP);
        return query.list();
      }
