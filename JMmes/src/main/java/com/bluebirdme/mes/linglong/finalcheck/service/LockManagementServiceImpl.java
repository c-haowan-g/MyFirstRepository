...
import com.bluebirdme.mes.linglong.finalcheck.entity.LockManagement;
...
public class LockManagementServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		LockManagement lockmanagement=(LockManagement) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), LockManagement.class);
        		//lockmanagement.setCreated_time_t(new Date()); 
        		list.add(lockmanagement);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		LockManagement lockmanagement =(LockManagement) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), LockManagement.class);
        		//lockmanagement.setCreated_time_t(new Date()); 
        		lockmanagement.setAgenc_no_s(FactoryConstant.JING_MEN_CODE);
        		lockmanagement.setS_factory_s(LLWebUtils.factory());
        		lockmanagement.setRecord_flag_s("A");
        		lockmanagement.setCreated_by_s(LLWebUtils.getUser().userName);
        		if(lockmanagement.getReas_cn_s().equals(null)||lockmanagement.getReas_cn_s().equals("")){
            		lockmanagement.setReas_code_s(lockmanagement.getControl_rules_s());
            		}
            		else if(lockmanagement.getControl_rules_s().equals(null)||lockmanagement.getControl_rules_s().equals("")){
            			lockmanagement.setReas_code_s(lockmanagement.getReas_cn_s());
            		}
        		list.add(lockmanagement);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		LockManagement lockmanagement=(LockManagement) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), LockManagement.class);
        		//lockmanagement.setCreated_time_t(new Date()); 
        		if(lockmanagement.getReas_cn_s().equals(null)||lockmanagement.getReas_cn_s().equals("")){
            		lockmanagement.setReas_code_s(lockmanagement.getControl_rules_s());
            		}
        		...
        		update(lockmanagement);
        	}
        }
    
    }
    @SuppressWarnings("unchecked")
	public List<Map<String, Object>> getreas() throws Exception {
		String factoryString = LLWebUtils.factory();
		String agenc_no_sString = FactoryConstant.JING_MEN_CODE;
		SQLQuery query = createSQLQuery(getSQL("getreas"));
