...
import com.bluebirdme.mes.linglong.quickcheck.entity.ReasWarn;
...
public class ReasWarnServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		ReasWarn reaswarn=(ReasWarn) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), ReasWarn.class);
        		//reaswarn.setCreated_time_t(new Date()); 
        		list.add(reaswarn);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		ReasWarn reaswarn =(ReasWarn) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), ReasWarn.class);
        		//reaswarn.setCreated_time_t(new Date()); 
        		list.add(reaswarn);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		ReasWarn reas=(ReasWarn) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), ReasWarn.class);
        		reas.setChanged_by_s(LLWebUtils.getUser().getUserName());
        		reas.setChanged_time_t(new Date()); 
        		update(reas);
        	}
        }
