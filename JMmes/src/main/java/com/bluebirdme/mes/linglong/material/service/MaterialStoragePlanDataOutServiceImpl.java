...
import com.bluebirdme.mes.linglong.material.entity.MaterialStoragePlanDataOut;
...
public class MaterialStoragePlanDataOutServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MaterialStoragePlanDataOut.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MaterialStoragePlanDataOut materialstorageplandataout=(MaterialStoragePlanDataOut) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MaterialStoragePlanDataOut.class);
        		//materialstorageplandataout.setCreated_time_t(new Date()); 
        		list.add(materialstorageplandataout);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStoragePlanDataOut materialstorageplandataout =(MaterialStoragePlanDataOut) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MaterialStoragePlanDataOut.class);
        		//materialstorageplandataout.setCreated_time_t(new Date()); 
        		list.add(materialstorageplandataout);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStoragePlanDataOut materialstorageplandataout=(MaterialStoragePlanDataOut) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MaterialStoragePlanDataOut.class);
        		//materialstorageplandataout.setCreated_time_t(new Date()); 
        		update(materialstorageplandataout);
        	}
        }
