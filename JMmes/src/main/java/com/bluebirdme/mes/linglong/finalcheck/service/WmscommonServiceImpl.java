...
import com.bluebirdme.mes.linglong.finalcheck.entity.Wmscommon;
...
public class WmscommonServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Wmscommon wmscommon=(Wmscommon) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Wmscommon.class);
        		//wmscommon.setCreated_time_t(new Date()); 
        		list.add(wmscommon);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Wmscommon wmscommon =(Wmscommon) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Wmscommon.class);
        		//wmscommon.setCreated_time_t(new Date()); 
        		list.add(wmscommon);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Wmscommon wmscommon=(Wmscommon) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Wmscommon.class);
        		//wmscommon.setCreated_time_t(new Date()); 
        		update(wmscommon);
        	}
        }
