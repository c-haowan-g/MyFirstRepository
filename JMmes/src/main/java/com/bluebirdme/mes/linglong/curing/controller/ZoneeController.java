...
import com.bluebirdme.mes.linglong.curing.service.ZoneeServiceImpl;
...
@RestController
@RequestMapping("curing/basedata/storagezonemanager")
public class ZoneeController extends FunctionalController<Zonee> {

	@Resource
	ZoneeServiceImpl szoneSrv;

	@Override
	protected BaseServiceImpl<Zonee> getService() {
		return szoneSrv;
	}

	@Log(code = "StorageZone_Save", value = "保存库区数据的增删改", target = LogType.DB)
	@RequestMapping("saveChanges")
	public Response saveChanges(@RequestBody ZoneeDTO storageZone) throws Exception {
	...
	public String zone() throws Exception {
		List<Zonee> test = szoneSrv.getZone();
