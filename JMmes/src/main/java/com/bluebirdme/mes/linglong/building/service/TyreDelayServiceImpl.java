...
import com.bluebirdme.mes.linglong.building.entity.TyreDelay;
...
public class TyreDelayServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		TyreDelay tyredelay=(TyreDelay) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), TyreDelay.class);
        		//tyredelay.setCreated_time_t(new Date()); 
        		list.add(tyredelay);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		TyreDelay tyredelay =(TyreDelay) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), TyreDelay.class);
        		//tyredelay.setCreated_time_t(new Date()); 
        		list.add(tyredelay);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		TyreDelay tyredelay=(TyreDelay) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), TyreDelay.class);
        		//tyredelay.setCreated_time_t(new Date()); 
        		update(tyredelay);
        	}
        }
