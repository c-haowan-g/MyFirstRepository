 ...
 */
public class SortTrimLogExportHandler extends DefaultExcelExportHandler {
    ...
    @SuppressWarnings({ "rawtypes", "unchecked", "null" })
   	...
   	public List<Object> getRows(Filter filter, Page page, String serviceName)
   			throws Exception {
    	SortTrimLogServiceImpl service = (SortTrimLogServiceImpl) SpringContext
   				.getBean(StringUtils
   						.firstCharToLowerCase("sortTrimLogServiceImpl"));
   		...
   		for (int i = 0; i < entityList.size(); i++) {
			...
			if (entityList.get(i).containsKey("etype_i")){
				//如果存在，可以直接put新的键值对。新的值会自动覆盖之前的。
				entityList.get(i).put("etype_i", exportCombox.getDictname(entityList.get(i).get("etype_i").toString()));
			}
   		}
   		if (entityList != null) {
   			for (int i = 0; i < entityList.size(); i++) {
