...
import com.bluebirdme.mes.linglong.common.entity.MixJobCheckTemp;
...
public class MixJobCheckTempServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixJobCheckTemp mixjobchecktemp=(MixJobCheckTemp) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixJobCheckTemp.class);
        		//mixjobchecktemp.setCreated_time_t(new Date()); 
        		list.add(mixjobchecktemp);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixJobCheckTemp mixjobchecktemp =(MixJobCheckTemp) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixJobCheckTemp.class);
        		mixjobchecktemp.setS_factory_s("0");
        		mixjobchecktemp.setCreated_by_s(WebUtils.getUser().userCode);
        		mixjobchecktemp.setCreated_name_s(WebUtils.getUser().userName);
        		mixjobchecktemp.setCreated_time_t(new Date()); 
        		list.add(mixjobchecktemp);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixJobCheckTemp mixjobchecktemp=(MixJobCheckTemp) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixJobCheckTemp.class);
        		mixjobchecktemp.setS_factory_s("0");
        		mixjobchecktemp.setChanged_by_s(WebUtils.getUser().userCode);
        		mixjobchecktemp.setChanged_name_s(WebUtils.getUser().userName);
        		mixjobchecktemp.setChanged_time_t(new Date()); 
        		update(mixjobchecktemp);
        	}
        }
    
    }
    public List findMaxCheckItem(String check_note_s)
    {
      SQLQuery query = getSession().createSQLQuery(getSQL("findMaxCheckItem"));
      query.setParameter("check_note_s", check_note_s);
