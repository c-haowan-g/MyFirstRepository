...
import com.bluebirdme.mes.linglong.material.entity.MaterialStoragePlanDataProd;
...
public class MaterialStoragePlanDataProdServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MaterialStoragePlanDataProd.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MaterialStoragePlanDataProd materialstorageplandataprod=(MaterialStoragePlanDataProd) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MaterialStoragePlanDataProd.class);
        		//materialstorageplandataprod.setCreated_time_t(new Date()); 
        		list.add(materialstorageplandataprod);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStoragePlanDataProd materialstorageplandataprod =(MaterialStoragePlanDataProd) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MaterialStoragePlanDataProd.class);
        		//materialstorageplandataprod.setCreated_time_t(new Date()); 
        		list.add(materialstorageplandataprod);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStoragePlanDataProd materialstorageplandataprod=(MaterialStoragePlanDataProd) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MaterialStoragePlanDataProd.class);
        		//materialstorageplandataprod.setCreated_time_t(new Date()); 
        		update(materialstorageplandataprod);
        	}
        }
