...
import com.bluebirdme.mes.linglong.mix.entity.PlmMixMix;
...
public class PlmMixMixServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmMixMix plmmixmix=(PlmMixMix) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmMixMix.class);
        		//plmmixmix.setCreated_time_t(new Date()); 
        		list.add(plmmixmix);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixMix plmmixmix =(PlmMixMix) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmMixMix.class);
        		//plmmixmix.setCreated_time_t(new Date()); 
        		list.add(plmmixmix);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixMix plmmixmix=(PlmMixMix) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmMixMix.class);
        		//plmmixmix.setCreated_time_t(new Date()); 
        		update(plmmixmix);
        	}
        }
