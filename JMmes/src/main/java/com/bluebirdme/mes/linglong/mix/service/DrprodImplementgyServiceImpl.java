...
import com.bluebirdme.mes.linglong.mix.entity.DrprodImplementgy;
...
public class DrprodImplementgyServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return DrprodImplementgy.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		DrprodImplementgy drprodimplementgy=(DrprodImplementgy) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), DrprodImplementgy.class);
        		//drprodimplementgy.setCreated_time_t(new Date()); 
        		list.add(drprodimplementgy);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		DrprodImplementgy drprodimplementgy =(DrprodImplementgy) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), DrprodImplementgy.class);
        		//drprodimplementgy.setCreated_time_t(new Date()); 
        		list.add(drprodimplementgy);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		DrprodImplementgy drprodimplementgy=(DrprodImplementgy) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), DrprodImplementgy.class);
        		//drprodimplementgy.setCreated_time_t(new Date()); 
        		update(drprodimplementgy);
        	}
        }
