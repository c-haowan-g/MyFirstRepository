...
public class MixMaterialReturnServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MixMaterialReturn.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixMaterialReturn mixmaterialreturn=(MixMaterialReturn) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixMaterialReturn.class);
        		//mixmaterialreturn.setCreated_time_t(new Date()); 
        		list.add(mixmaterialreturn);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixMaterialReturn mixmaterialreturn =(MixMaterialReturn) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixMaterialReturn.class);
        		//mixmaterialreturn.setCreated_time_t(new Date()); 
        		list.add(mixmaterialreturn);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixMaterialReturn mixmaterialreturn=(MixMaterialReturn) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixMaterialReturn.class);
        		//mixmaterialreturn.setCreated_time_t(new Date()); 
        		update(mixmaterialreturn);
        	}
        }
