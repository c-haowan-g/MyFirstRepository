...
public class MixRfridScanningrateServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MixRfridScanningrate.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixRfridScanningrate mixrfridscanningrate=(MixRfridScanningrate) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixRfridScanningrate.class);
        		//mixrfridscanningrate.setCreated_time_t(new Date()); 
        		list.add(mixrfridscanningrate);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixRfridScanningrate mixrfridscanningrate =(MixRfridScanningrate) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixRfridScanningrate.class);
        		//mixrfridscanningrate.setCreated_time_t(new Date()); 
        		list.add(mixrfridscanningrate);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixRfridScanningrate mixrfridscanningrate=(MixRfridScanningrate) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixRfridScanningrate.class);
        		//mixrfridscanningrate.setCreated_time_t(new Date()); 
        		update(mixrfridscanningrate);
        	}
        }
    ...
    {
      List list = null;
      if (filter.get("equip_name_s") != null)
        list = getAll1(filter);
      else {
        list = getAll2(filter);
      }
    ...
    public List<Iterators.Map<String, Object>> getAll1(Filter filter)
    {
      ...
      query.setParameter("equip_name_s", filter.get("equip_name_s"));
    ...
    public List<Iterators.Map<String, Object>> getAll2(Filter filter)
    {
