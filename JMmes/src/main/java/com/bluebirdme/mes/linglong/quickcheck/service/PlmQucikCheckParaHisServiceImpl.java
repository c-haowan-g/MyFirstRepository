...
import com.bluebirdme.mes.linglong.quickcheck.entity.PlmQucikCheckParaHis;
...
public class PlmQucikCheckParaHisServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return PlmQucikCheckParaHis.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmQucikCheckParaHis parahis=(PlmQucikCheckParaHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmQucikCheckParaHis.class);
        		//parahis.setCreated_time_t(new Date()); 
        		list.add(parahis);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmQucikCheckParaHis parahis =(PlmQucikCheckParaHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmQucikCheckParaHis.class);
        		//parahis.setCreated_time_t(new Date()); 
        		list.add(parahis);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmQucikCheckParaHis parahis=(PlmQucikCheckParaHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmQucikCheckParaHis.class);
        		//parahis.setCreated_time_t(new Date()); 
        		update(parahis);
        	}
        }
