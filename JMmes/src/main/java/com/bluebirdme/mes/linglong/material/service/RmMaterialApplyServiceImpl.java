...
public class RmMaterialApplyServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		RmMaterialApply rmmaterialapply=(RmMaterialApply) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), RmMaterialApply.class);
        		//rmmaterialapply.setCreated_time_t(new Date()); 
        		list.add(rmmaterialapply);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		RmMaterialApply rmmaterialapply =(RmMaterialApply) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), RmMaterialApply.class);
        		rmmaterialapply.setCreated_by_s(LLWebUtils.getUser().userCode);
        		rmmaterialapply.setCreated_by_s(LLWebUtils.getUser().userName);
        		rmmaterialapply.setCreated_time_t(new Date()); 
        		list.add(rmmaterialapply);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		RmMaterialApply rmmaterialapply=(RmMaterialApply) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), RmMaterialApply.class);
        		
        		update(rmmaterialapply);
        	}
        }
