...
import com.bluebirdme.mes.linglong.mix.entity.MixAlarmData;
...
public class MixAlarmDataServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixAlarmData mixalarmdata=(MixAlarmData) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixAlarmData.class);
        		//mixalarmdata.setCreated_time_t(new Date()); 
        		list.add(mixalarmdata);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixAlarmData mixalarmdata =(MixAlarmData) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixAlarmData.class);
        		//mixalarmdata.setCreated_time_t(new Date()); 
        		list.add(mixalarmdata);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixAlarmData mixalarmdata=(MixAlarmData) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixAlarmData.class);
        		//mixalarmdata.setCreated_time_t(new Date()); 
        		update(mixalarmdata);
        	}
        }
