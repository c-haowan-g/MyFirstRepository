...
import com.bluebirdme.mes.linglong.halfpart.entity.DelayHandleRecord;
...
public class DelayHandleRecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return DelayHandleRecord.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		DelayHandleRecord delayhandlerecord=(DelayHandleRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), DelayHandleRecord.class);
        		//delayhandlerecord.setCreated_time_t(new Date()); 
        		list.add(delayhandlerecord);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		DelayHandleRecord delayhandlerecord =(DelayHandleRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), DelayHandleRecord.class);
        		//delayhandlerecord.setCreated_time_t(new Date()); 
        		list.add(delayhandlerecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		DelayHandleRecord delayhandlerecord=(DelayHandleRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), DelayHandleRecord.class);
        		//delayhandlerecord.setCreated_time_t(new Date()); 
        		update(delayhandlerecord);
        	}
        }
