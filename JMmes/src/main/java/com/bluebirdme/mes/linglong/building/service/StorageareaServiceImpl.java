...
import com.bluebirdme.mes.linglong.building.entity.Storagearea;
...
public class StorageareaServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Storagearea storagearea=(Storagearea) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Storagearea.class);
        		//storagearea.setCreated_time_t(new Date()); 
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Storagearea storagearea =(Storagearea) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Storagearea.class);
        		//storagearea.setCreated_time_t(new Date()); 
        		list.add(storagearea);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Storagearea storagearea=(Storagearea) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Storagearea.class);
        		//storagearea.setCreated_time_t(new Date()); 
        		update(storagearea);
        	}
        }
