...
import com.bluebirdme.mes.linglong.mix.entity.PlmMixWeightExecHis;
...
public class PlmMixWeightExecHisServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return PlmMixWeightExecHis.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmMixWeightExecHis plm_mixweightexechis=(PlmMixWeightExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmMixWeightExecHis.class);
        		//plm_mixweightexechis.setCreated_time_t(new Date()); 
        		list.add(plm_mixweightexechis);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixWeightExecHis plm_mixweightexechis =(PlmMixWeightExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmMixWeightExecHis.class);
        		//plm_mixweightexechis.setCreated_time_t(new Date()); 
        		list.add(plm_mixweightexechis);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixWeightExecHis plm_mixweightexechis=(PlmMixWeightExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmMixWeightExecHis.class);
        		//plm_mixweightexechis.setCreated_time_t(new Date()); 
        		update(plm_mixweightexechis);
        	}
        }
