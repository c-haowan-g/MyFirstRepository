...
import com.bluebirdme.mes.linglong.material.entity.MaterialStockPlanResult;
...
public class MaterialStockPlanResultServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MaterialStockPlanResult.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MaterialStockPlanResult materialstockplanresult=(MaterialStockPlanResult) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MaterialStockPlanResult.class);
        		//materialstockplanresult.setCreated_time_t(new Date()); 
        		list.add(materialstockplanresult);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStockPlanResult materialstockplanresult =(MaterialStockPlanResult) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MaterialStockPlanResult.class);
        		//materialstockplanresult.setCreated_time_t(new Date()); 
        		list.add(materialstockplanresult);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStockPlanResult materialstockplanresult=(MaterialStockPlanResult) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MaterialStockPlanResult.class);
        		//materialstockplanresult.setCreated_time_t(new Date()); 
        		update(materialstockplanresult);
        	}
        }
   ...
   /* public String lock(Long[] ids) throws Exception {
        try {
          ...
          String result = portType.reciveInventoryCalculateResultByMES(ids[0].toString(), "347001", 
      ...
      public String onsave(Long[] ids) throws Exception {
        try {
          MESWSImplementService wsexinterface = new MESWSImplementService();
          INVWSInterface portType = wsexinterface.getMESWSImplementPort();
          StringBuffer errorBuffer = new StringBuffer();
          StringBuffer resultBuffer = new StringBuffer();
          String result = portType.reciveInventorySaveResultByMES(ids[0].toString(), "347001", 
              (LLWebUtils.getUser()).userCode, (LLWebUtils.getUser()).userName, null, null, null, null, null);
          if (result.equals("OK")) {
            resultBuffer.append(buildcommonMsg(Integer.valueOf(1), "+ ids[0].toString() + ","+ result"));
          } else {
