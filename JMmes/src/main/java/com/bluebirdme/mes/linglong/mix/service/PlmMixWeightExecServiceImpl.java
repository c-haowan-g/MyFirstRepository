...
import com.bluebirdme.mes.linglong.mix.entity.PlmMixWeightExec;
...
public class PlmMixWeightExecServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmMixWeightExec plmmixweightexec=(PlmMixWeightExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmMixWeightExec.class);
        		//plmmixweightexec.setCreated_time_t(new Date()); 
        		list.add(plmmixweightexec);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixWeightExec plmmixweightexec =(PlmMixWeightExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmMixWeightExec.class);
        		//plmmixweightexec.setCreated_time_t(new Date()); 
        		list.add(plmmixweightexec);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixWeightExec plmmixweightexec=(PlmMixWeightExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmMixWeightExec.class);
        		//plmmixweightexec.setCreated_time_t(new Date()); 
        		update(plmmixweightexec);
        	}
        }
