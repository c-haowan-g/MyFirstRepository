...
public class MixStoragePlanDataOutServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MixStoragePlanDataOut.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixStoragePlanDataOut mixstorageplandataout=(MixStoragePlanDataOut) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixStoragePlanDataOut.class);
        		//mixstorageplandataout.setCreated_time_t(new Date()); 
        		list.add(mixstorageplandataout);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixStoragePlanDataOut mixstorageplandataout =(MixStoragePlanDataOut) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixStoragePlanDataOut.class);
        		//mixstorageplandataout.setCreated_time_t(new Date()); 
        		list.add(mixstorageplandataout);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixStoragePlanDataOut mixstorageplandataout=(MixStoragePlanDataOut) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixStoragePlanDataOut.class);
        		//mixstorageplandataout.setCreated_time_t(new Date()); 
        		update(mixstorageplandataout);
        	}
        }
