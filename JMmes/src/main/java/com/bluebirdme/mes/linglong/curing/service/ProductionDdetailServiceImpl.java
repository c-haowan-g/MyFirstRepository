...
public class ProductionDdetailServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return ProductionDdetail.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		ProductionDdetail productionddetail=(ProductionDdetail) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), ProductionDdetail.class);
        		//productionddetail.setCreated_time_t(new Date()); 
        		list.add(productionddetail);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		ProductionDdetail productionddetail =(ProductionDdetail) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), ProductionDdetail.class);
        		//productionddetail.setCreated_time_t(new Date()); 
        		list.add(productionddetail);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		ProductionDdetail productionddetail=(ProductionDdetail) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), ProductionDdetail.class);
        		//productionddetail.setCreated_time_t(new Date()); 
        		update(productionddetail);
        	}
        }
