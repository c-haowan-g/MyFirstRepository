...
import com.bluebirdme.mes.linglong.material.entity.IntSapDn;
...
public class IntSapDnServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		IntSapDn intsapdn=(IntSapDn) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), IntSapDn.class);
        		//intsapdn.setCreated_time_t(new Date()); 
        		list.add(intsapdn);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		IntSapDn intsapdn =(IntSapDn) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), IntSapDn.class);
        		//intsapdn.setCreated_time_t(new Date()); 
        		list.add(intsapdn);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		IntSapDn intsapdn=(IntSapDn) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), IntSapDn.class);
        		//intsapdn.setCreated_time_t(new Date()); 
        		update(intsapdn);
        	}
        }
