...
import com.bluebirdme.mes.linglong.mix.entity.PlmMixWeight;
...
public class PlmMixWeightServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmMixWeight plmmixweight=(PlmMixWeight) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmMixWeight.class);
        		//plmmixweight.setCreated_time_t(new Date()); 
        		list.add(plmmixweight);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixWeight plmmixweight =(PlmMixWeight) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmMixWeight.class);
        		//plmmixweight.setCreated_time_t(new Date()); 
        		list.add(plmmixweight);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixWeight plmmixweight=(PlmMixWeight) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmMixWeight.class);
        		//plmmixweight.setCreated_time_t(new Date()); 
        		update(plmmixweight);
        	}
        }
     ...
     * @throws Exception
     */
    public String changeweight(String[] ids, String[] weight) throws Exception {
        if (ids.length <= 0)
          throw new Exception("未选择数据"); 
        if (weight.length <= 0)
          throw new Exception("未填写重量"); 
        ...
        for (int i = 0; i < ids.length; i++) {
          ...
          BigDecimal bdchangeweight = new BigDecimal(changeweight);
          bdchangeweight = bdchangeweight.setScale(2, 4);
          WSImplementService wsexinterface = new WSImplementService();
          WSExInterface portType = wsexinterface.getWSImplementPort();
          sb.append("称重修改结果："+ portType.reciveMixRecipeWeightByMES(null, null, id, bdchangeweight, 
    ...
    public String submitweighing(String[] ids, String[] weight) throws Exception {
        if (ids.length <= 0)
          throw new Exception("请选择一条数据并输入称量设定范围上下限"); 
        if (weight.length <= 0)
          throw new Exception("未填写重量"); 
        StringBuffer sb = new StringBuffer();
        BigDecimal bdchangeweight = new BigDecimal(weight[0]);
        BigDecimal bdchangeweight1 = new BigDecimal(weight[1]);
        WSImplementService wsexinterface = new WSImplementService();
        WSExInterface portType = wsexinterface.getWSImplementPort();
        /*sb.append("称量修改结果"+ portType.reciveMixRecipeWeightRangeByMES(null, null, ids[0], bdchangeweight, bdchangeweight1, 
              (LLWebUtils.getUser()).userCode, (LLWebUtils.getUser()).userName));*/
        return sb.toString();
      }
