...
import com.bluebirdme.mes.linglong.finalcheck.entity.TyreReasCountRecord;
...
public class TyreReasCountRecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return TyreReasCountRecord.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		TyreReasCountRecord tyrereascountrecord=(TyreReasCountRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), TyreReasCountRecord.class);
        		//tyrereascountrecord.setCreated_time_t(new Date()); 
        		list.add(tyrereascountrecord);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		TyreReasCountRecord tyrereascountrecord =(TyreReasCountRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), TyreReasCountRecord.class);
        		//tyrereascountrecord.setCreated_time_t(new Date()); 
        		list.add(tyrereascountrecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		TyreReasCountRecord tyrereascountrecord=(TyreReasCountRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), TyreReasCountRecord.class);
        		//tyrereascountrecord.setCreated_time_t(new Date()); 
        		update(tyrereascountrecord);
        	}
        }
