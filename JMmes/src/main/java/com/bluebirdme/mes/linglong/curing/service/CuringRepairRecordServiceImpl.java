...
import com.bluebirdme.mes.linglong.curing.entity.CuringRepairRecord;
...
public class CuringRepairRecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return CuringRepairRecord.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		CuringRepairRecord curingrepairrecord=(CuringRepairRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), CuringRepairRecord.class);
        		//curingrepairrecord.setCreated_time_t(new Date()); 
        		list.add(curingrepairrecord);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		CuringRepairRecord curingrepairrecord =(CuringRepairRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), CuringRepairRecord.class);
        		//curingrepairrecord.setCreated_time_t(new Date()); 
        		list.add(curingrepairrecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		CuringRepairRecord curingrepairrecord=(CuringRepairRecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), CuringRepairRecord.class);
        		//curingrepairrecord.setCreated_time_t(new Date()); 
        		update(curingrepairrecord);
        	}
        }
