...
public class RawMaterialWarningServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return RawMaterialWarning.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		RawMaterialWarning rawmaterialwarning=(RawMaterialWarning) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), RawMaterialWarning.class);
        		//rawmaterialwarning.setCreated_time_t(new Date()); 
        		list.add(rawmaterialwarning);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		RawMaterialWarning rawmaterialwarning =(RawMaterialWarning) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), RawMaterialWarning.class);
        		//rawmaterialwarning.setCreated_time_t(new Date()); 
        		list.add(rawmaterialwarning);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		RawMaterialWarning rawmaterialwarning=(RawMaterialWarning) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), RawMaterialWarning.class);
        		//rawmaterialwarning.setCreated_time_t(new Date()); 
        		update(rawmaterialwarning);
        	}
        }
