...
import com.bluebirdme.mes.linglong.curing.entity.CuringCancelWork;
...
public class CuringCancelWorkServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		CuringCancelWork curingcancelwork=(CuringCancelWork) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), CuringCancelWork.class);
        		//curingcancelwork.setCreated_time_t(new Date()); 
        		list.add(curingcancelwork);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		CuringCancelWork curingcancelwork =(CuringCancelWork) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), CuringCancelWork.class);
        		//curingcancelwork.setCreated_time_t(new Date()); 
        		list.add(curingcancelwork);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		CuringCancelWork curingcancelwork=(CuringCancelWork) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), CuringCancelWork.class);
        		//curingcancelwork.setCreated_time_t(new Date()); 
        		update(curingcancelwork);
        	}
        }
