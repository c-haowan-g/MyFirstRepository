...
import com.bluebirdme.mes.linglong.mix.entity.MixRecipeWeightProd;
...
public class MixRecipeWeightProdServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MixRecipeWeightProd.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixRecipeWeightProd mixrecipeweightprod=(MixRecipeWeightProd) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixRecipeWeightProd.class);
        		//mixrecipeweightprod.setCreated_time_t(new Date()); 
        		list.add(mixrecipeweightprod);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixRecipeWeightProd mixrecipeweightprod =(MixRecipeWeightProd) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixRecipeWeightProd.class);
        		//mixrecipeweightprod.setCreated_time_t(new Date()); 
        		list.add(mixrecipeweightprod);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixRecipeWeightProd mixrecipeweightprod=(MixRecipeWeightProd) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixRecipeWeightProd.class);
        		//mixrecipeweightprod.setCreated_time_t(new Date()); 
        		update(mixrecipeweightprod);
        	}
        }
