...
import com.bluebirdme.mes.linglong.material.entity.MaterialStoragePlanData;
...
public class MaterialStoragePlanDataServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MaterialStoragePlanData.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MaterialStoragePlanData materialstorageplandata=(MaterialStoragePlanData) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MaterialStoragePlanData.class);
        		//materialstorageplandata.setCreated_time_t(new Date()); 
        		list.add(materialstorageplandata);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStoragePlanData materialstorageplandata =(MaterialStoragePlanData) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MaterialStoragePlanData.class);
        		//materialstorageplandata.setCreated_time_t(new Date()); 
        		list.add(materialstorageplandata);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStoragePlanData materialstorageplandata=(MaterialStoragePlanData) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MaterialStoragePlanData.class);
        		//materialstorageplandata.setCreated_time_t(new Date()); 
        		update(materialstorageplandata);
        	}
        }
