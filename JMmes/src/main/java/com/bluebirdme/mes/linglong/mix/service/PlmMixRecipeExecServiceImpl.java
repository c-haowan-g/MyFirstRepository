...
import com.bluebirdme.mes.linglong.mix.entity.PlmMixRecipeExec;
...
public class PlmMixRecipeExecServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmMixRecipeExec plmmixrecipeexec=(PlmMixRecipeExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmMixRecipeExec.class);
        		//plmmixrecipeexec.setCreated_time_t(new Date()); 
        		list.add(plmmixrecipeexec);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixRecipeExec plmmixrecipeexec =(PlmMixRecipeExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmMixRecipeExec.class);
        		//plmmixrecipeexec.setCreated_time_t(new Date()); 
        		list.add(plmmixrecipeexec);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixRecipeExec plmmixrecipeexec=(PlmMixRecipeExec) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmMixRecipeExec.class);
        		//plmmixrecipeexec.setCreated_time_t(new Date()); 
        		update(plmmixrecipeexec);
        	}
        }
