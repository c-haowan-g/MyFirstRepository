...
import com.bluebirdme.mes.linglong.finalcheck.entity.Appearancejudgework;
...
public class AppearancejudgeworkServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return Appearancejudgework.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Appearancejudgework appearancejudgework=(Appearancejudgework) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Appearancejudgework.class);
        		//appearancejudgework.setCreated_time_t(new Date()); 
        		list.add(appearancejudgework);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Appearancejudgework appearancejudgework =(Appearancejudgework) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Appearancejudgework.class);
        		//appearancejudgework.setCreated_time_t(new Date()); 
        		list.add(appearancejudgework);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Appearancejudgework appearancejudgework=(Appearancejudgework) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Appearancejudgework.class);
        		//appearancejudgework.setCreated_time_t(new Date()); 
        		update(appearancejudgework);
        	}
        }
