...
public class MixInventoryServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixInventory mixinventory=(MixInventory) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixInventory.class);
        		//mixinventory.setCreated_time_t(new Date()); 
        		list.add(mixinventory);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixInventory mixinventory =(MixInventory) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixInventory.class);
        		//mixinventory.setCreated_time_t(new Date()); 
        		list.add(mixinventory);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	List<Map<String, Object>> mixInventory = rockWellEntityDTO.getUpdated();
			for(Map<String, Object> newitem:mixInventory){
				MixInventory item = (MixInventory)MapTrunPojo.mapToEntity(newitem, getEntity());
        		// 修改存在状态
				//item.setState_code_s("001001");
				// 修改创建时间
				Date date = new Date();
...
//        	    //将MAP转换为实体
//        		MixInventory mixinventory=(MixInventory) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixInventory.class);
//        		//mixinventory.setCreated_time_t(new Date()); 
//        		update(mixinventory);
//        	}
        }
