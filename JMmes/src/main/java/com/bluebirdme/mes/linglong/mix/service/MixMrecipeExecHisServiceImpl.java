...
import com.bluebirdme.mes.linglong.mix.entity.MixMrecipeExecHis;
 ...
 * PLM密炼配方执行历史表
 * @author 陈国强
 * @Date 2020-09-16 09:54:49
 */
@Service
@Transactional(rollbackFor=Exception.class)
public class MixMrecipeExecHisServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MixMrecipeExecHis.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixMrecipeExecHis mixmrecipeexechis=(MixMrecipeExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixMrecipeExecHis.class);
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixMrecipeExecHis mixmrecipeexechis =(MixMrecipeExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixMrecipeExecHis.class);
        		//mixmrecipeexechis.setCreated_time_t(new Date()); 
        		list.add(mixmrecipeexechis);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixMrecipeExecHis mixmrecipeexechis=(MixMrecipeExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixMrecipeExecHis.class);
        		//mixmrecipeexechis.setCreated_time_t(new Date()); 
