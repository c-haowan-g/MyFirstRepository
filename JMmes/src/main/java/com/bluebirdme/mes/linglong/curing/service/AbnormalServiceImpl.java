...
import com.bluebirdme.mes.linglong.curing.entity.*;
...
public class AbnormalServiceImpl extends RockWellUDABaseServiceImpl<Object> {
     ...
     * 查询异常信息数据
     */
    public Map<String, Object> searchReplenishData(Filter filter, Page page) throws Exception {
    ...
    public String guhua(Map<String, Object> rockWellEntityDTO) throws Exception {
        List<Map<String, Object>> insertedList = (List<Map<String, Object>>) rockWellEntityDTO.get("selectedrows");
        boolean flag=false;
        if (!ListUtils.isEmpty(insertedList)) {
            ...
            for (Map<String, Object> r : insertedList) {
                ...
                List<Map<String,Object>> list=productionDetailByCuring(r,beginTime,endTime);
                for(int i=0;i<list.size();i++){
                    ...
                    productionDdetail.setSpare1_s("252006");//备用字段1 被用于存放工序数据：252006硫化/成型252005
                    productionDdetail.setSpare2_s("03");// 备用字段2 被用于存放区分报产，冲产数据:01:报产，02：冲产，03：异常
                ...
                String updateWorkorderresultSql = String.format(
                        "update at_c_mm_workorderresult t set t.solidif_yflag_s = '1', t.spare1_s = '%s'"
                                ...
                                + " and t.curing_closemoulddate_t between to_date('%s','YYYY-mm-dd HH24:MI:SS')  "
                                + " and to_date('%s','YYYY-mm-dd HH24:MI:SS') ", serialno,
                        LLConstant.Record_Flag.AVAILIABLE, LLWebUtils.factory(), LLConstant.JING_MEN_CODE, materialCode,
                        classId, productionver, beginTime, endTime);
                System.out.println(updateWorkorderresultSql);
                // 更新硫化产出表。更新（1）固化标识为已固化（2）固化流水号
                RockWellUtils.execSQL(updateWorkorderresultSql);

                String workorderno = getSerid();
                ...
                mesSapProduction.setProduct_type_s("03");// 01:报产，02：冲产，03：异常
...
*/				mesSapProduction.setBatch_id_s(serialno);// 设置流水号
     ...
     */
    @RockWellTransactional
    public List<Map<String,Object>> productionDetailByCuring(Map<String,Object> workOrderResultReportDTO,String beginTime,String endTime) throws Exception {
        List<Map<String, Object>> selectRows = (List<Map<String, Object>>) workOrderResultReportDTO.get("selectedrows");
    ...
    public List<Object> getWorkOrderCode(String code, String type) {
        ...
        if (StringUtils.isBlank(type)) {
            sql = "select SERID_S from AT_INT_SAP_MesSapProduction where SERID_S like '%" + code + "%'";
        } else {
            sql = "select SERID_S from AT_INT_SAP_MesSapProduction where PRODUCT_TYPE_S='" + type
                    + "' and SERID_S like '%" + code + "%'";
        }
     ...
     * @throws Exception
     */
    public List<SapReturnResultSimple> guhuaData(String beginTime, String endTime, String material_code_s)
            throws Exception {

        String sql = "SELECT " + "0 atr_key,'' serid_s,MATERIAL_CODE_S, " + "	PRODUCTIONVER_S, " + "	MATERIAL_NAME_S, "
                + "	CLASS_ID_S CLASS_ID_S, " + "	COUNT (1) TOTAL " + "FROM " + "	AT_C_MM_SAPRETURNRESULT " + "WHERE "
                + "	CURING_CLOSEMOULDDATE_T BETWEEN to_date('" + beginTime + "','yyyy-MM-dd HH24:mi:ss') "
                + "AND to_date('" + endTime + "','yyyy-MM-dd HH24:mi:ss') " + "AND ( " + "	SOLIDIF_YFLAG_S = '0'  ) "
                + " AND (SPARE6_S = '1' OR SPARE3_S='1') and S_FACTORY_S='" + LLWebUtils.factory()
                + "' and MATERIAL_CODE_S like '%" + material_code_s + "%'" + "GROUP BY " + "	MATERIAL_CODE_S, "
