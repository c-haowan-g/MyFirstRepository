...
import com.bluebirdme.mes.linglong.building.entity.StorageAreaHis;
...
public class StorageAreaHisServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		StorageAreaHis storageareahis=(StorageAreaHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), StorageAreaHis.class);
        		//storageareahis.setCreated_time_t(new Date()); 
        		list.add(storageareahis);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		StorageAreaHis storageareahis =(StorageAreaHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), StorageAreaHis.class);
        		//storageareahis.setCreated_time_t(new Date()); 
        		list.add(storageareahis);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		StorageAreaHis storageareahis=(StorageAreaHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), StorageAreaHis.class);
        		//storageareahis.setCreated_time_t(new Date()); 
        		update(storageareahis);
        	}
        }
