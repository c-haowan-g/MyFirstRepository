...
import com.bluebirdme.mes.linglong.quickcheck.entity.PlmQucikCheckPara;
...
public class PlmQucikCheckParaServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return PlmQucikCheckPara.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmQucikCheckPara plmqucikcheckpara=(PlmQucikCheckPara) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmQucikCheckPara.class);
        		//plmqucikcheckpara.setCreated_time_t(new Date()); 
        		list.add(plmqucikcheckpara);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmQucikCheckPara plmqucikcheckpara =(PlmQucikCheckPara) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmQucikCheckPara.class);
        		//plmqucikcheckpara.setCreated_time_t(new Date()); 
        		list.add(plmqucikcheckpara);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmQucikCheckPara plmqucikcheckpara=(PlmQucikCheckPara) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmQucikCheckPara.class);
        		//plmqucikcheckpara.setCreated_time_t(new Date()); 
        		update(plmqucikcheckpara);
        	}
        }
