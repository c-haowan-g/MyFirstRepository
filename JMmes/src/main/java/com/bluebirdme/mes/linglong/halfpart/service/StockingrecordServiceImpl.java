...
import com.bluebirdme.mes.linglong.halfpart.entity.Stockingrecord;
...
public class StockingrecordServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Stockingrecord stockingrecord=(Stockingrecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Stockingrecord.class);
        		//stockingrecord.setCreated_time_t(new Date()); 
        		list.add(stockingrecord);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Stockingrecord stockingrecord =(Stockingrecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Stockingrecord.class);
        		//stockingrecord.setCreated_time_t(new Date()); 
        		list.add(stockingrecord);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Stockingrecord stockingrecord=(Stockingrecord) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Stockingrecord.class);
        		//stockingrecord.setCreated_time_t(new Date()); 
    ...
    /*public List<Map<String, Object>> datagridLog(Filter filter, Page page) throws Exception {
    ...
    */
    ...
    public List<Map<String, Object>> searchMaterielgroup(){
