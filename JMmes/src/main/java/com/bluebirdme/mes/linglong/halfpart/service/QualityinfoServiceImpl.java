...
public class QualityinfoServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public String saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Qualityinfo qualityinfo=(Qualityinfo) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Qualityinfo.class);
        		qualityinfo.setRecord_flag_s(LLConstant.Record_Flag.DELETED);
        		qualityinfo.setChanged_by_s(LLWebUtils.getUser().getUserName());
        		qualityinfo.setChanged_time_t(new Date());
        		qualityinfo.setSite_num(qualityinfo.getSite_num());
        		update(qualityinfo);
        	}
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Qualityinfo qualityinfo =(Qualityinfo) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Qualityinfo.class);
        		String sickelephantsNo = qualityinfo.getSickelephants_no_s();//病象编号
        		List<Map<String, Object>> QualityinfoMap = datagrid_rows(sickelephantsNo,LLWebUtils.factory());
        		if(QualityinfoMap.size() > 0){
        			return "error";
        		}
        		
        		qualityinfo.setAgenc_no_s(LLConstant.JING_MEN_CODE);
        		qualityinfo.setS_factory_s(LLWebUtils.factory());
        		qualityinfo.setRecord_flag_s(LLConstant.Record_Flag.AVAILIABLE);
        		qualityinfo.setCreated_by_s(LLWebUtils.getUser().getUserName());
        		qualityinfo.setCreated_time_t(new Date());
        		qualityinfo.setChanged_by_s(LLWebUtils.getUser().getUserName());
        		qualityinfo.setChanged_time_t(new Date());
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Qualityinfo qualityinfo=(Qualityinfo) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Qualityinfo.class);
        		qualityinfo.setChanged_by_s(LLWebUtils.getUser().getUserName());
        		qualityinfo.setChanged_time_t(new Date());
        		update(qualityinfo);
        	}
    ...
    public List<Map<String, Object>> datagrid_rows(String sickelephantsNo,String factory){
    	SQLQuery query = createSQLQuery(getSQL("searchSickNo"));
    	query.setParameter("sickelephants_no_s", sickelephantsNo);
    	query.setParameter("s_factory_s", factory);
    	return query.list();
    }
    
    public List<Map<String, Object>> getMaxNo(){
    	SQLQuery query = createSQLQuery(getSQL("getMaxNo"));
