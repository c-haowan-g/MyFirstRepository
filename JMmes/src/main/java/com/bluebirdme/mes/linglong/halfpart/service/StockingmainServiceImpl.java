...
import com.bluebirdme.mes.linglong.halfpart.entity.Stockingmain;
...
public class StockingmainServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		Stockingmain stockingmain=(Stockingmain) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), Stockingmain.class);
        		//stockingmain.setCreated_time_t(new Date()); 
        		list.add(stockingmain);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		Stockingmain stockingmain =(Stockingmain) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), Stockingmain.class);
        		//stockingmain.setCreated_time_t(new Date()); 
        		list.add(stockingmain);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		Stockingmain stockingmain=(Stockingmain) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), Stockingmain.class);
        		//stockingmain.setCreated_time_t(new Date()); 
        		update(stockingmain);
        	}
        }
