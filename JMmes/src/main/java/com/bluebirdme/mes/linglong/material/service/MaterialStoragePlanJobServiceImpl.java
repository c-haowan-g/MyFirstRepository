...
import com.bluebirdme.mes.linglong.material.entity.MaterialStoragePlanJob;
...
public class MaterialStoragePlanJobServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return MaterialStoragePlanJob.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MaterialStoragePlanJob materialstorageplanjob=(MaterialStoragePlanJob) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MaterialStoragePlanJob.class);
        		//materialstorageplanjob.setCreated_time_t(new Date()); 
        		list.add(materialstorageplanjob);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStoragePlanJob materialstorageplanjob =(MaterialStoragePlanJob) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MaterialStoragePlanJob.class);
        		//materialstorageplanjob.setCreated_time_t(new Date()); 
        		list.add(materialstorageplanjob);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MaterialStoragePlanJob materialstorageplanjob=(MaterialStoragePlanJob) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MaterialStoragePlanJob.class);
        		//materialstorageplanjob.setCreated_time_t(new Date()); 
        		update(materialstorageplanjob);
        	}
        }
