...
public class SapReturnResultServiceImpl extends RockWellUDABaseServiceImpl<Object> {
		...
		if (!ListUtils.isEmpty(insertedList)) {
			...
			for (Map<String, Object> r : insertedList) {
				...
				for(int i=0;i<list.size();i++){
					...
					productionDdetail.setSpare2_s("02");// 备用字段2 被用于存放区分报产，冲产数据:01:报产，02：冲产
					productionDdetailServiceImpl.save(productionDdetail);
				}
				String updatesapreturnresultSql = String.format(
						"update AT_C_MM_SAPRETURNRESULT t set t.solidif_yflag_s = '1', t.spare1_s = '%s'"
								+ " where t.record_flag_s = '%s' " + " and t.s_factory_s='%s' "
								+ " and t.agenc_no_s='%s'  " + " and t.material_code_s='%s' "
								+ " and t.CLASS_ID_S='%s' " + " and t.productionver_s='%s' "
