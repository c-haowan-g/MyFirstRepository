...
public class RawMaterialInventoryStateServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return RawMaterialInventoryState.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		RawMaterialInventoryState rawmaterialinventorystate=(RawMaterialInventoryState) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), RawMaterialInventoryState.class);
        		//rawmaterialinventorystate.setCreated_time_t(new Date()); 
        		list.add(rawmaterialinventorystate);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		RawMaterialInventoryState rawmaterialinventorystate =(RawMaterialInventoryState) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), RawMaterialInventoryState.class);
        		//rawmaterialinventorystate.setCreated_time_t(new Date()); 
        		list.add(rawmaterialinventorystate);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		RawMaterialInventoryState rawmaterialinventorystate=(RawMaterialInventoryState) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), RawMaterialInventoryState.class);
        		//rawmaterialinventorystate.setCreated_time_t(new Date()); 
        		update(rawmaterialinventorystate);
        	}
        }
