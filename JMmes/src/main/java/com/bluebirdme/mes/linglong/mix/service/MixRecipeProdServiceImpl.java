...
import com.bluebirdme.mes.linglong.mix.entity.MixRecipeProd;
...
public class MixRecipeProdServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		MixRecipeProd mixrecipeprod=(MixRecipeProd) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), MixRecipeProd.class);
        		//mixrecipeprod.setCreated_time_t(new Date()); 
        		list.add(mixrecipeprod);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		MixRecipeProd mixrecipeprod =(MixRecipeProd) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), MixRecipeProd.class);
        		//mixrecipeprod.setCreated_time_t(new Date()); 
        		list.add(mixrecipeprod);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		MixRecipeProd mixrecipeprod=(MixRecipeProd) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), MixRecipeProd.class);
        		//mixrecipeprod.setCreated_time_t(new Date()); 
        		update(mixrecipeprod);
        	}
        }
