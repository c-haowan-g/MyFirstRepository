...
import com.bluebirdme.mes.linglong.mix.entity.PlmMixRecipeExecHis;
...
public class PlmMixRecipeExecHisServiceImpl extends  RockWellUDABaseServiceImpl<Object> {
    ...
    protected Class getEntity() {
        return PlmMixRecipeExecHis.class;
    }
    ...
    public void saveChanges(RockWellEntityDTO rockWellEntityDTO) throws Exception{
        ...
        if (!ListUtils.isEmpty(rockWellEntityDTO.getDeleted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getDeleted().size(); i++) {
        		//将MAP转换为实体
        		PlmMixRecipeExecHis mixmrecipeexechis=(PlmMixRecipeExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getDeleted().get(i), PlmMixRecipeExecHis.class);
        		//mixmrecipeexechis.setCreated_time_t(new Date()); 
        		list.add(mixmrecipeexechis);
        	}
        	delete(list);
        }
        if (!ListUtils.isEmpty(rockWellEntityDTO.getInserted())) {
        	...
        	for (int i = 0; i < rockWellEntityDTO.getInserted().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixRecipeExecHis mixmrecipeexechis =(PlmMixRecipeExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getInserted().get(i), PlmMixRecipeExecHis.class);
        		//mixmrecipeexechis.setCreated_time_t(new Date()); 
        		list.add(mixmrecipeexechis);
        	}
        	save(list);
        ...
        */
        if (!ListUtils.isEmpty(rockWellEntityDTO.getUpdated())) {
        	for (int i = 0; i < rockWellEntityDTO.getUpdated().size(); i++) {
        	    //将MAP转换为实体
        		PlmMixRecipeExecHis mixmrecipeexechis=(PlmMixRecipeExecHis) MapTrunPojo.mapToEntity(rockWellEntityDTO.getUpdated().get(i), PlmMixRecipeExecHis.class);
        		//mixmrecipeexechis.setCreated_time_t(new Date()); 
        		update(mixmrecipeexechis);
        	}
        }
