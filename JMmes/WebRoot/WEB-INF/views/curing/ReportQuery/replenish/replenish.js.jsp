...
<script type="text/javascript">
  ...
  //固化标识转换
    ...
    function onMesSapDgBeforeLoad(param){
    	param["filter[product_type_s]"]="01";
    }
	...
	function guhua_filter() {
		...
		ext.ajax.get("curing/report/productionSubsidiaryQuery/guhua?beginTime=" + beginTime
	...
	function save(){
        ...
        Utils.each(updated,function(i,e){
        	_updated.push({atr_key:e.atr_key,material_code_s:e.material_code_s,material_name_s:e.material_name_s,class_id_s:e.class_id_s,total:e.actual_quantity_f,productionver_s:e.pro_version_s,record_flag_s:'D'});
        });
        ...
        ext.ajax.sendJson("lh/scgl/sap_return_result/guhua", {replenish:true,inserted:_inserted,updated:_updated,deleted:_deleted}, function(data) {
            if(data.code==OK){
            	ext.tip.suc("保存成功");
            	filter();
            	return;
