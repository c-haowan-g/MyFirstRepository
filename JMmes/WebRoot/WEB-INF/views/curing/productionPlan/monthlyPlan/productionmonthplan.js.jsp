...
<script type="text/javascript">
    ...
    var saveChanges=path+"curing/productionPlan/monthPlan/saveChanges";
    ...
    function onBeforeEdit(index,row){
    	editIndex=index;
    	if(row.atr_key){
    		   ext.ajax.post("curing/productionPlan/monthPlan/checkDailyPlan",{month:row.month_i,materialCode:row.material_code_s},function(data){
   ...
   // 验证月计划数量更改状态
    function onChange(nv,ov){
    	...
    	var row=$("#sapproductionplan_dg").datagrid("getRows")[index];
    	  $.ajax({
	 		url:"curing/productionPlan/monthPlan/checkdayplan?month_i="+row.month_i+"&material_code_s="+row.material_code_s,
	 		type:"get",
	 		dataType:"json",
	 		async:false,
