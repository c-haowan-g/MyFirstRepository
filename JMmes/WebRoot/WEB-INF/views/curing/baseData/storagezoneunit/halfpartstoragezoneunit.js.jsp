...
<script type="text/javascript">
    ...
    var saveStorageZoneChanges="curing/basedata/storagezonemanager/saveChanges";
    var saveUnitMentChanges=path+"curing/basedata/storageunitmanager/saveChanges";
  ...
  /**
		 ...
		 if(total>0){ 
			$('#halfpart_storagezone_dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.datagrid({
						url:'curing/basedata/storageunitmanager/unit?zonekey='+row.storage_zone_key,
						fitColumns:true,
						singleSelect:true,
						rownumbers:true,
						loadMsg:'加载中...',
	...
	*/
    $(function(){
    $('#halfpart_storagezone_dg').datagrid({
        url: 'curing/basedata/storagezonemanager/searchZoneByCategoryAndZoneName',
        fit:true,
        fitColumns:true,
	...
	function zone_unit(){
		 ...
		 var url = 'curing/basedata/storageunitmanager/unit?zonekey='+rowData.storage_zone_key+'&materialGroup='+materialGroup;
	...
	function getMaterialCodeByName(record){
	      ...
	      $.ajax({
	    	    url:'curing/productionplan/dailyplan/getDataByMaterialName?materialName='+record.MATERIALDESC_S,
	      		type:"get",
	      		dataType:"json",
	      		async:false,
	      		success:function(data){
    ...
    var filter=function(){
    	...
    	category=$('#category').textbox('getValue');
    	 $.ajax({
		 		url:'curing/basedata/storagezonemanager/searchZoneByCategoryAndZoneName?category='+category+'&description='+description,
		 		type:"post",
		 		dataType:"json",
		 		data : JSON,
	...
	function zoneAndMaterialGroupJudge(record){
		...
		if(rowData==null){
			...
			var judgeUrl='curing/basedata/storagezonemanager/zoneAndMaterialGroupJudge?zoneName='+zoneName+'&materialGroup='+record.DICTCODE_S;
			$.ajax({
				    url:judgeUrl,
			        type:'get',
			        dataType:'json',
