<!-- 
    ...
    作者      :刘朋
    时间      :2019-07-09 09:04:57 
    页面名称:PLM动平衡标准执行表
    文件关联:
        CommonPLMDynamicBalanceExec.java
        CommonPLMDynamicBalanceExecController.java
        CommonPLMDynamicBalanceExecServiceImpl.java
        CommonPLMDynamicBalanceExecServiceImpl.xml
 -->
<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%@ include file="../../../inc/meta.jsp"%>
<%@ include file="commonplmdynamicbalanceexec.js.jsp"%>
<script type="text/javascript" src="dict/323.js"></script>

<body class="easyui-layout" fit="true"> 
    <div data-options="region:'center',split:true,border:false" >
        <div id="CommonPLMDynamicBalanceExec_dg_toolbar">
            <form id="CommonPLMDynamicBalanceExec_search_form" class="form" >
        ...
        <table id="commonplmdynamicbalanceexec_dg" class="easyui-datagrid" style="width:100%"
            data-options="  
                fit:true,
                fitColumns:false,
                pagination:true,
                rownumbers: true,
                singleSelect: false,
                toolbar: '#CommonPLMDynamicBalanceExec_dg_toolbar',
                url: 'common/plm/commonplmdynamicbalanceexec/datagridAll',
                striped:true,
                method: 'get' 
                ">
            <thead>
                <tr>
                    ...
                    <!-- 审核状态：0-未审核、1-已审核 -->
                    <th data-options="field:'check_state_s',formatter:checkStateFormatter,styler:checkStateCellStyler,editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalance.column.check_state_s" /></th>
                    <!-- 下发标识（下发成功后置逻辑删除）：0-未下发、1-已下发 -->
                    <th data-options="field:'send_state_s',formatter:letdownFormatter,styler:letdownCellStyler,editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.send_state_s" /></th>
                    <!-- 下发人姓名 -->
                    <th data-options="field:'send_name_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.send_name_s" /></th>
                    <!-- 下发时间 -->
                    <th data-options="field:'send_time_t',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.send_time_t" /></th>
                    <!-- 配方启停人 -->
                    <th data-options="field:'recipe_by_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.recipe_by_s" /></th>
                    <!-- 配方启停时间 -->
                    <th data-options="field:'recipe_time_t',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.recipe_time_t" /></th>
					<!-- 数据同步处理标记：0-未处理、1-已处理 -->
                    <th data-options="field:'sync_hand_flag_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.sync_hand_flag_s" /></th>
                    <!-- 数据同步处理：A-新增、U-修改、D-删除 -->
                    <th data-options="field:'sync_flag_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.sync_flag_s" /></th>
                    <!-- 同步时间 -->
                    <th data-options="field:'sync_create_time_t',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.sync_create_time_t" /></th>
					<!-- 数据同步处理时间 -->
                    <th data-options="field:'sync_hand_time_t',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.sync_hand_time_t" /></th>
                    <!-- 数据同步处理反馈 -->
                    <th data-options="field:'sync_hand_msg_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.sync_hand_msg_s" /></th>
                   <!-- 设备类别：TBROE-全钢配套、TBRRE-全钢零售、PCROE-半钢配套、PCRRE-半钢零售 -->
                    <th data-options="field:'category_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.category_s" /></th>
                    <!-- 规格 -->
                    <th data-options="field:'specification_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.specification_s" /></th>
                    <!-- 层级 -->
                    <th data-options="field:'hierarchy_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.hierarchy_s" /></th>
                    <!-- 描述 -->
                    <th data-options="field:'description_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.description_s" /></th>
                    <!-- 花纹 -->
                    <th data-options="field:'pattern_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.pattern_s" /></th>
                  <!-- SAP号 -->
                    <th data-options="field:'sapcode_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.sapcode_s" /></th>
                  <!-- 气压 -->
                    <th data-options="field:'pressure_s',editor:{type:'textbox'}"><spring:message code="commonplmdynamicbalanceexec.column.pressure_s" /></th>
                    <!-- 负荷 -->
