<!--
 作者      :李新宇
    ...
    文件关联:
        StorageSpace.java
        StorageZone.java
        HalfPartZoneController.java
        HalfPartZoneServiceImpl.java
        HalfPartZoneServiceImpl.xml
...
<div data-options="region:'center'">
    <div id="Halfpart_zone_dg_toolbar">
        ...
        <auth:button code="HALFPART_STORAGE_ZONE_UNIT"><a class="easyui-linkbutton btn-green"
                                                          data-options="iconCls:'icon-add'" onclick="zone_unit_qu()">库区维护</a></auth:button>
        <auth:button code="HALFPART_STORAGE_ZONE_UNIT"><a class="easyui-linkbutton btn-green"
                                                          data-options="iconCls:'icon-add'"
                                                          onclick="zone_unit()"><spring:message
                code="storage.unit"/></a></auth:button>
    </div>
    <table id="halfpart_storagezone_dg" class="easyui-datagrid"
        ...
        <tr>
            ...
            <!-- 操作人uda  -->
            <th data-options="field:'operater_s',width:80,sortable:true,editor:{type:'textbox',options:{disabled:true}}">
                操作人
            </th>
            <!-- 操作时间uda  -->
            <th data-options="field:'operatetime_t',width:100,sortable:true,editor:{type:'textbox',options:{disabled:true}}">
                操作时间
            </th>
        </tr>
        </thead>
...
<div id="halfpart_unit_manage_dg" class="easyui-dialog" title="库位维护"
    ...
    <div id="Halfpart_unit_dg_toolbar">
        <form class="form" id="Halfpart_unit_search_form" method="post">
            <div data-options="region:'center'">
                <auth:button code="HALFPART_STORAGE_UNIT_ADD"><a class="easyui-linkbutton btn-green"
                                                                 data-options="iconCls:'icon-add'" onclick="unit_add()"><spring:message
                        code="button.add"/></a></auth:button>
                <auth:button code="HALFPART_STORAGE_UNIT_EDIT"><a class="easyui-linkbutton btn-red"
                                                                  data-options="iconCls:'icon-edit'"
                                                                  onclick="unit_edit()"><spring:message
                        code="button.edit"/></a></auth:button>
                <auth:button code="HALFPART_STORAGE_UNIT_REMOVE"><a class="easyui-linkbutton btn-black"
                                                                    data-options="iconCls:'icon-remove'"
                                                                    onclick="unit_remove()"><spring:message
                        code="button.delete"/></a></auth:button>
                <auth:button code="HALFPART_STORAGE_UNIT_SAVE"><a class="easyui-linkbutton btn-blue"
                                                                  data-options="iconCls:'icon-save'"
                                                                  onclick="unit_save()"><spring:message
                        code="button.save"/></a></auth:button>
                <auth:button code="HALFPART_STORAGE_UNIT_CANCEL"><a class="easyui-linkbutton btn-yellow"
                                                                    data-options="iconCls:'icon-no'"
                                                                    onclick="unit_cancel()"><spring:message
    ...
    <table id="halfpart_storageunit_dg" class="easyui-datagrid"
        ...
        <tr>
            ...
            <!-- 库位名称 -->
            <th data-options="field:'storage_unit_name',sortable:true,width:120,editor:{type:'textbox',options:{ required:true, events:{blur: checkUnit}}}">
                库位名称
            </th>
            <!-- 库位描述 -->
            ...
            <!-- 库存容量 -->
            <th data-options="field:'max_storage_duration',width:80,sortable:true,editor:{type:'numberspinner',options:{min:1}}">
                库存容量
            </th>
            <!-- 当前库存 -->
            <th data-options="field:'current_storage_i',width:80,sortable:true,editor:{type:'textbox',options:{readonly:true}}">
                当前库存
            </th>
            <!-- 存放物料名称 -->
            <th data-options="field:'inventory_materialname_s',width:100,sortable:true,hidden:true,editor:{type:'textbox',options:{}}">
                <spring:message code="sapproductionplan.column.material_name_s"/></th>
            <!-- 存放物料编码 -->
            <th data-options="field:'inventory_materialcode_s',width:100,sortable:true,hidden:true,editor:{type:'textbox',options:{}}">
                <spring:message code="sapproductionplan.column.material_code_s"/></th>
            <!-- 最后入库时间 uda-->
            <th data-options="field:'lastintime_t',width:120,sortable:true,editor:{type:'textbox',options:{readonly:true}}">
                最后入库时间
            </th>
            <!-- 启用/停用 -->
            ...
            <!-- 操作人 -->
            <th data-options="field:'operater_s',width:90,sortable:true,editor:{type:'textbox',options:{disabled:true}}">
                操作人
            </th>
            <!-- 操作时间 -->
            <th data-options="field:'operatetime_t',sortable:true,editor:{type:'textbox',options:{disabled:true}}">
                操作时间
            </th>
        </tr>
        </thead>
...
<div id="manage_dg" class="easyui-dialog" title="库区维护"
     data-options="closed:true ,
                     width:window.innerWidth,
		             height:window.innerHeight,
		             maximizable:true,
		             ...
		             ">
    <auth:button code="HALFPART_STORAGE_ZONE_ADD"><a class="easyui-linkbutton btn-green"
                                                     data-options="iconCls:'icon-add'"
                                                     onclick="zone_add()"><spring:message
            code="button.add"/></a></auth:button>
    <auth:button code="HALFPART_STORAGE_ZONE_EDIT"><a class="easyui-linkbutton btn-red"
                                                      data-options="iconCls:'icon-edit'"
                                                      onclick="zone_edit()"><spring:message
            code="button.edit"/></a></auth:button>
    <auth:button code="HALFPART_STORAGE_ZONE_REMOVE"><a class="easyui-linkbutton btn-black"
                                                        data-options="iconCls:'icon-remove'"
                                                        onclick="zone_remove()"><spring:message
            code="button.delete"/></a></auth:button>
    <auth:button code="HALFPART_STORAGE_ZONE_SAVE"><a class="easyui-linkbutton btn-blue"
                                                      data-options="iconCls:'icon-save'"
                                                      onclick="zone_save()"><spring:message
            code="button.save"/></a></auth:button>
    <auth:button code="HALFPART_STORAGE_ZONE_CANCEL"><a class="easyui-linkbutton btn-yellow"
                                                        data-options="iconCls:'icon-no'"
                                                        onclick="zone_cancel()"><spring:message
            code="button.cancel"/></a> </auth:button>



    <table id="halfpart_storagezone_dg2" class="easyui-datagrid"
           data-options="
                        fit:true,
                        fitColumns:true,
                        pagination:false,
