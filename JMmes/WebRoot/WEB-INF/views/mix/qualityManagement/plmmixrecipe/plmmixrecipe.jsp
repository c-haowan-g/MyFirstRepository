<!-- 
    ...
    作者      :栾和源
    时间      :2020-08-08 14:03:08 
    页面名称:PLM密炼配方集成表
    文件关联:
        PlmMixRecipe.java
        PlmMixRecipeController.java
        PlmMixRecipeServiceImpl.java
        PlmMixRecipeServiceImpl.xml
 -->
<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%@ include file="../../../../inc/meta.jsp"%>
<%@ include file="plmmixrecipe.js.jsp"%>
...
<body class="easyui-layout" fit="true">
	<div data-options="region:'center',split:true,border:true">
		<div class="easyui-layout" data-options="fit:true">
			<div data-options="region:'center',split:true,border:true">
				<div id="PlmMixRecipe_dg_toolbar">
            	<form id="PlmMixRecipe_search_form" class="form" >
            ...
            <auth:button code="MIXRECIPE_WEIGHTMANAGEEDIT">
			<a class="easyui-linkbutton btn-black" data-options="iconCls:'icon-edit'" onclick="edit()">称量调整</a>
			</auth:button>
			<auth:button code="MIXRECIPE_WEIGHTMANAGEEDIT_COMMIT">
			<a class="easyui-linkbutton btn-black" data-options="iconCls:'icon-supplier'" onclick="changeweight()">提交称量调整</a>
			</auth:button>
			<auth:button code="MIXRECIPE_AUDIT"> 
			<a class="easyui-linkbutton btn-green" data-options="iconCls:'icon-shenhe'" onclick="audit()">审核</a>
			</auth:button>
			<auth:button code="MIXRECIPE_CANCELAUDIT">
			<a class="easyui-linkbutton btn-red" data-options="iconCls:'icon-fanshenhe'" onclick="cancelaudit()">撤销审核</a>
			</auth:button>
			<auth:button code="MIXRECIPE_LETDOWN">
			<a class="easyui-linkbutton btn-black" data-options="iconCls:'icon-supplier'" onclick="downward()">下发</a>
			</auth:button>
			<auth:button code="MIXRECIPE_START">
			<a class="easyui-linkbutton btn-green" data-options="iconCls:'icon-shenhe'" onclick="startrecipe()">待用</a>
			</auth:button>
			<auth:button code="MIXRECIPE_STOP">
			<a class="easyui-linkbutton btn-red" data-options="iconCls:'icon-fanshenhe'" onclick="stoprecipe()">停用</a>
			</auth:button>
        </div>
        <table id="plmmixrecipe_dg" class="easyui-datagrid" style="width:100%"
            data-options="  
                fit:true,
                fitColumns:false,
                pagination:true,
                rownumbers: true,
                singleSelect: true,
                toolbar: '#PlmMixRecipe_dg_toolbar',
                url: '',
                onSelect:selectrow,
                striped:true,
                method: 'get' 
	...
	<div data-options="region:'west',split:true,collapsed:true"
		title="配方结构" style="width: 250px;">
		<div id="tree" style="width: 35%">
			<ul id="tree_menu" class="ztree"
				data-options="
				onClick: clicktree">
			</ul>
		</div>
	...
	<div id="menu_dlg" class="easyui-dialog" title="物料列表-双击选择物料"
			style="width: 1000px; height: 90%;"
			data-options="
				            resizable:true,
				   			modal:true,
					    	onOpen:menuDlgOpen,
		...
		<div data-options="region:'center',split:true,border:false"
			...
			<table id="materialcodedlg" class="easyui-datagrid"
				style="width: 100%; height: 100%;"
				data-options="  
							fit:true,
						    fitColumns:true,
							rowStyler:materialRowStylerChange,
							pagination:true,
							rownumbers: true,
							emptyMsg: '<span><spring:message code='tip.noData'/></span>',
							singleSelect: true,
							striped:true,
						    singleSelect: true,
						    onDblClickRow:materialcodedlgdbclick,
			                url: 'common/plm/mixrecipe/getmaterialcode',
						    method: 'get',
					...
					<tr>
						...
						<!-- 是否有配方 -->
						<th
							data-options="field:'FLAG',width:50,editor:{type:'textbox',options:{required:true}}"><spring:message
								code="materials.column.flag" /></th>
						<!-- 品号 -->
						<th
							data-options="field:'MATERIALCODE_S',width:50,editor:{type:'textbox',options:{required:true}}"><spring:message
								code="materials.column.materialcode_s" /></th>
						<!-- 物料描述 -->
						<th
							data-options="field:'MATERIALDESC_S',width:150,editor:{type:'textbox',options:{required:true}}"><spring:message
								code="materials.column.materialdesc_s" /></th>
						<!-- 物料规格 -->
						<th
							data-options="field:'MATERIALSPEC_S',width:150,editor:{type:'textbox',options:{required:true}}"><spring:message
								code="materials.column.materialspec_s" /></th>
						<!-- 物料组 -->
						<th
							data-options="field:'MATGRP_S',formatter:matgrpformatter,width:150,editor:{type:'textbox',options:{required:true}}"><spring:message
								code="materials.column.matgrp_s" /></th>
					</tr>
					</thead>
			</table>
		</div>
