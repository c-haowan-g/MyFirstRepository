...
<script type="text/javascript">
	...
	//接收小料物料数据
	/*     var materialLHCodeList=[];
	 $.ajax({
	 url:"common/plm/mixrecipeedit/mixLHXLMaterialList",
	 type:"get",
	 dataType:"json",
	 async:false,
	 success:function(data){
	 materialLHCodeList=data;
	 }
	 }); */
	...
	//是否转换
	function produceReciveFormatter(v, r, i) {
		if (v == 1) {
			return '是';
		}
		return '否';
	...
	function onMaterialSelect1(record) {
		...
		ext.ajax
				.get(
						"mix/productionplan/mixdayplan/getAddInfoList2?materialcode="
								+ record.MATER_CODE_S,
						function(data) {
							if (data.code != OK) {
							...
							if (v.length == 1) {
								...
								$(
										getEditor1(index,
												"mixrecipe_recipe_code_s").target)
	...
	function onRecipeSelect1(record) {
		...
		if (record.mater_type_s == "LLOrdinaryBagBatch") {
			$(getEditor1(index, "plan_type_s").target).textbox("setValue",
					"051001");//计划类型-普通小料包
		}
		if (record.mater_type_s == "LLSulfurBagBatch") {
			$(getEditor1(index, "plan_type_s").target).textbox("setValue",
					"051002");//计划类型--硫磺小料包
		}
